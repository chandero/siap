...
where (select count(*) from siap.sms_control sc1 where sc1.tireuc_id =o.tireuc_id and sc1.reti_id = o.reti_id and sc1.repo_id = o.repo_id) = 0;"""
        ...
        db.withConnection { implicit connection => 
            ...
            _resultSet.map { reporte =>
                ...
                val _regexCellPhonePattern = "^[\\+]?[(]?3[0-9]{2}[)]?[-\\s\\.]?[0-9]{3}[-\\s\\.]?[0-9]{4,6}$".r
                reporte._6 match {
                    case _regexCellPhonePattern(cellPhoneNumber) => {
                        
                        SQL(_queryGuardar).on(
                            'tireuc_id -> reporte._1,
                            'reti_id -> reporte._2,
                            'repo_id -> reporte._3,
                            'smsc_message -> message,
                            'smsc_fecha -> new DateTime(),
                            'smsc_destino -> cellPhoneNumber,
                            'smsc_referencia -> cellPhoneNumber,
