...
case class Solicitud(a: SolicitudA, b: SolicitudB)

case class SolicitudAR(
    soli_id: Option[Long],
    soti_id: Option[Long],
    soti_descripcion: Option[String],
    soli_fecha: Option[DateTime],
...
object TipoSolicitud {
  ...
  val _set = {
    get[Option[Long]]("soti_id") ~
      ...
      case soti_id ~ soti_descripcion =>
        TipoSolicitud(soti_id, soti_descripcion)
    }
  }
}
...
class SolicitudRepository @Inject()(
    dbapi: DBApi,
  ...
  /**
    ...
    * @param empr_id: Long
    */
  def buscarPorRadicado(
      soli_radicado: String,
      empr_id: Long
  ): Option[Solicitud] = {
    db.withConnection { implicit connection =>
      val s = SQL(
  ...
  def buscarPorDescripcion(
      soli_descripcion: String,
      empr_id: Long
  ): Future[Iterable[Solicitud]] = Future[Iterable[Solicitud]] {
    db.withConnection { implicit connection =>
  ...
  def cuenta(empr_id: Long): Long = {
    db.withConnection { implicit connection =>
      val result = SQL(
        "SELECT COUNT(*) AS c FROM siap.solicitud WHERE soli_estado <> 9 and empr_id = {empr_id}"
      ).on(
          'empr_id -> empr_id
        )
  ...
  def todos(
    ...
    db.withConnection { implicit connection =>
      var _listBuffer = new ListBuffer[Solicitud]()
      val lsoli = SQL(
        """SELECT soli_id, soli_descripcion, soli_estado, usua_id FROM siap.solicitud s
                            LEFT JOIN siap.barrio b on s.barr_id = b.barr_id
                            LEFT JOIN siap.solicitud_tipo st ON st.soti_id = s.soti_id
  ...
  /**
    * Recuperar todos los Solicitud activas
    */
  def solis(empr_id: Long): Future[Iterable[Solicitud]] =
    Future[Iterable[Solicitud]] {
      db.withConnection { implicit connection =>
        var _listBuffer = new ListBuffer[Solicitud]()
        val lsoli = SQL(
          """SELECT soli_id, soli_descripcion, soli_estado, usua_id FROM siap.solicitud
                            LEFT JOIN siap.barrio b on s.barr_id = b.barr_id
                            LEFT JOIN siap.solicitud_tipo st ON st.soti_id = s.soti_id
  ...
  /**
    ...
    * @param empr_id: scala.Long
    */
  def buscarPorVencer(empr_id: scala.Long): Future[Iterable[Solicitud]] =
    Future[Iterable[Solicitud]] {
      db.withConnection { implicit connection =>
        var query: String =
          """SELECT *, (CASE WHEN s.soli_estado = 1 THEN 'PENDIENTE' WHEN s.soli_estado = 2 THEN 'EN SUPERVISOR' WHEN s.soli_estado = 3 THEN 'EN VISITA' WHEN s.soli_estado = 4 THEN 'EN CRONOGRAMA' WHEN s.soli_estado = 5 THEN 'EN INFORME' WHEN s.soli_estado = 6 THEN 'RESPONDIDA' END) as soli_estado_descripcion FROM siap.solicitud s 
  ...
  def crear(soli: Solicitud): Future[Long] = Future {
    db.withConnection { implicit connection =>
      ...
      val dias_festivos = SQL(
        "SELECT COUNT(*) FROM siap.festivo WHERE fest_dia BETWEEN {fecha_inicial} and {fecha_final}"
      ).on(
          'fecha_inicial -> fi,
          'fecha_final -> ff
        )
      ...
      var radicado: Int = SQL(
        "SELECT gene_numero FROM siap.general WHERE gene_id = {gene_id} and empr_id = {empr_id}"
      ).on(
          'gene_id -> 2,
  ...
  def borrar(soli_id: Long, empr_id: Long, usua_id: Long): Boolean = {
    db.withConnection { implicit connection =>
      ...
      val count: Long = SQL(
        "UPDATE siap.soli SET soli_estado = 9 WHERE soli_id = {soli_id} and empr_id = {empr_id}"
      ).on(
          'soli_id -> soli_id,
          'empr_id -> empr_id
        )
  ...
  def entregarSupervisor(
    ...
    db.withConnection { implicit connection =>
      ...
      val count: Long = SQL(
        "UPDATE siap.solicitud SET soli_fechasupervisor = {soli_fechasupervisor}, soli_estado = {soli_estado} WHERE soli_id = {soli_id} and empr_id = {empr_id}"
      ).on(
          'soli_id -> soli_id,
          'empr_id -> empr_id,
  ...
  def entregarFormatoRTE(
    ...
    db.withConnection { implicit connection =>
      ...
      // verificar si ya tiene asignado el consecutivo de atención
      val soli_consecutivo = SQL(
        """SELECT soli_consecutivo FROM siap.solicitud ss1 WHERE ss1.soli_id = {soli_id}"""
      ).on(
          'soli_id -> soli_id
        )
        .as(SqlParser.scalar[Int].singleOpt)
      soli_consecutivo match {
        case None =>
          var consecutivo: Int = SQL(
            "SELECT g.gene_numero FROM siap.general g WHERE g.gene_id = {gene_id}"
          ).on('gene_id -> 3).as(SqlParser.scalar[Int].single)
          consecutivo += 1
          SQL(
            ...
            .executeUpdate()
          val count: Long = SQL(
            "UPDATE siap.solicitud SET soli_consecutivo = {soli_consecutivo}, soli_fecharte = {soli_fecharte}, soli_estado = {soli_estado} WHERE soli_id = {soli_id} and empr_id = {empr_id}"
          ).on(
              'soli_id -> soli_id,
              'empr_id -> empr_id,
  ...
  def ingresarInforme(
    ...
    db.withConnection { implicit connection =>
      ...
      val count: Long = SQL(
        "UPDATE siap.solicitud SET soli_fechainforme = {soli_fechainforme}, soli_informe = {soli_informe}, soli_estado = {soli_estado} WHERE soli_id = {soli_id} and empr_id = {empr_id}"
      ).on(
          'soli_id -> soli_id,
          'empr_id -> empr_id,
  ...
  def asignarRte(
    ...
    db.withConnection { implicit connection =>
      ...
      val hora: LocalDateTime =
        new LocalDateTime(Calendar.getInstance().getTimeInMillis())
      val count: Long = SQL(
        "UPDATE siap.solicitud SET soli_fechaalmacen = {soli_fechaalmacen}, soli_numerorte = {soli_numerorte}, soli_estado = {soli_estado} WHERE soli_id = {soli_id} and empr_id = {empr_id}"
      ).on(
          'soli_id -> soli_id,
          'empr_id -> empr_id,
  ...
  def ingresarRespuesta(
    ...
    db.withConnection { implicit connection =>
      ...
      val count: Long = SQL(
        "UPDATE siap.solicitud SET soli_fecharespuesta = {soli_fecharespuesta}, soli_respuesta = {soli_respuesta}, soli_estado = {soli_estado} WHERE soli_id = {soli_id} and empr_id = {empr_id}"
      ).on(
          'soli_id -> soli_id,
          'empr_id -> empr_id,
        ...
        buscarPorIdCompleto(soli_id) match {
          case Some(s) =>
            ...
            val hora: LocalDateTime =
              new LocalDateTime(Calendar.getInstance().getTimeInMillis())
            val count: Long = SQL(
              "UPDATE siap.solicitud SET soli_fecharespuesta = {soli_fecharespuesta}, soli_estado = {soli_estado} WHERE soli_id = {soli_id} and empr_id = {empr_id}"
            ).on(
                'soli_id -> soli_id,
                'empr_id -> empr_id,
                'soli_fecharespuesta -> hora,
            ...
            s.soli_aprobada match {
              case Some(true) =>
                compiledFile = REPORT_DEFINITION_PATH + "siap_carta_respuesta_solicitud_aprobada.jasper"
                if (con_firma == 1) {
                  val firma: URL = new URL(
                    "file",
                    "localhost",
                    REPORT_DEFINITION_PATH + "firma.png"
                  )
                  ...
                  val firma: URL = new URL(
                    "file",
                    "localhost",
                    REPORT_DEFINITION_PATH + "blanco.png"
                  )
                  println("URL Firma : " + firma)
                  reportParams.put("FIRMA", firma)
                }
              case Some(false) =>
                compiledFile = REPORT_DEFINITION_PATH + "siap_carta_respuesta_solicitud_negada.jasper"
              case None => None
            }
            ...
            reportParams
              .put("FECHA_LARGA", Utility.fechaatexto(s.soli_fecharespuesta))
            reportParams.put("CODIGO_RESPUESTA", s.soli_codigorespuesta.get)
            reportParams.put("CIUDAD_CORTA", ciudad)
            reportParams
              .put("FECHA_RADICADO_LARGA", Utility.fechaatexto(s.soli_fecha))
            ...
            reportParams
              .put("LUMINARIAS_LETRAS", N2T.convertirLetras(luminarias))
            reportParams.put("GERENTE", gerente)
            if (editable) {
              println("Es Editable")
              var docExporter = new JRDocxExporter()
              val jasperPrint = JasperFillManager
                .fillReport(compiledFile, reportParams, connection)
              ...
              docExporter.setExporterOutput(
                new SimpleOutputStreamExporterOutput(ostream)
              );
