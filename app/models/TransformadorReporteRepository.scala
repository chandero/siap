...
class TransformadorReporteRepository @Inject()(
    dbapi: DBApi,
    eventoService: EventoRepository,
    elementoService: ElementoRepository,
    empresaService: EmpresaRepository,
    usuarioService: UsuarioRepository,
    aapService: TransformadorRepository
  ...
  def validarReporteDiligenciado(
    ...
    db.withConnection { implicit connection =>
      ...
      if (reti_id > 1) {
        val reportes = SQL(
          """SELECT r.repo_consecutivo FROM siap.transformador_reporte r 
  ...
  def cuenta(empr_id: scala.Long): scala.Long = {
    db.withConnection { implicit connection =>
      val result = SQL(
        "SELECT COUNT(*) AS c FROM siap.transformador_reporte WHERE empr_id = {empr_id} and rees_id < 9"
      ).on(
          'empr_id -> empr_id
        )
        .as(SqlParser.scalar[scala.Long].single)
    ...
    Future[Iterable[Reporte]] {
      db.withConnection { implicit connection =>
        ...
        reps.map { r =>
          ...
          val eventos = SQL(
            """SELECT * FROM siap.transformador_reporte_evento WHERE repo_id = {repo_id} and even_estado < 8"""
          ).on(
              'repo_id -> r.repo_id
            )
            .as(Evento.eventoSet *)
  ...
  def reportes(empr_id: scala.Long): Future[Iterable[Reporte]] =
    Future[Iterable[Reporte]] {
      db.withConnection { implicit connection =>
        var _list: ListBuffer[Reporte] = new ListBuffer[Reporte]()
        val reps = SQL("""SELECT * FROM siap.transformador_reporte r 
                                          WHERE r.empr_id = {empr_id} 
    ...
    Future[Iterable[Vencido]] {
      db.withConnection { implicit connection =>
        SQL(
         ...
         o.orig_descripcion, r.repo_nombre, r.repo_telefono, r.repo_direccion, b.barr_descripcion, r.repo_descripcion, r.rees_id from siap.transformador_reporte r
        inner join siap.transformador_reporte_tipo rt on rt.reti_id = r.reti_id
        left join siap.barrio b on b.barr_id = r.barr_id
    ...
    db.withConnection { implicit connection =>
      val r = SQL(
        """SELECT * FROM siap.transformador_reporte r
                            INNER JOIN siap.transformador_reporte_tipo t on r.reti_id = t.reti_id
                            ...
                            LEFT JOIN siap.barrio b on r.barr_id = b.barr_id
                            INNER JOIN siap.transformador_reporte_estado e on r.rees_id = e.rees_id
  ...
  def buscarPorId(repo_id: scala.Long): Option[Reporte] = {
    db.withConnection { implicit connection =>
      val r = SQL("""SELECT * FROM siap.transformador_reporte r
    ...
    db.withConnection { implicit connection =>
      ...
      reps.map { r =>
        val adicional = SQL(
          ...
          .as(ReporteAdicional.reporteAdicionalSet.singleOpt)
        val eventos = SQL(
          """SELECT * FROM siap.transformador_reporte_evento WHERE repo_id = {repo_id} """
        ).on(
            'repo_id -> r.repo_id
          )
          .as(Evento.eventoSet *)
  ...
  def buscarPorRangoFechaSolucion(
    ...
    db.withConnection { implicit connection =>
      SQL(
        "SELECT * FROM siap.transformador_reporte WHERE repo_fechasolucion BETWEEN {fecha_inicial} and {fecha_final} and empr_id = {empr_id}"
      ).on(
  ...
  def buscarPorEstado(
    ...
    db.withConnection { implicit connection =>
      SQL(
        "SELECT * FROM siap.transformador_reporte WHERE rees_id = {rees_id} and empr_id = {empr_id}"
      ).on(
          'rees_id -> rees_id,
          'empr_id -> empr_id
        )
  ...
  def buscarPorActividad(
    ...
    db.withConnection { implicit connection =>
      SQL(
        "SELECT * FROM siap.transformador_reporte WHERE acti_id = {acti_id} and empr_id = {empr_id}"
      ).on(
          'acti_id -> acti_id,
          'empr_id -> empr_id
        )
  ...
  def buscarPorOrigen(
    ...
    db.withConnection { implicit connection =>
      SQL(
        "SELECT * FROM siap.transformador_reporte WHERE orig_id = {orig_id} and empr_id = {empr_id}"
      ).on(
          'orig_id -> orig_id,
          'empr_id -> empr_id
        )
  ...
  def buscarAvanzado(
    ...
    db.withConnection { implicit connection =>
      SQL("SELECT * FROM siap.transformador_reporte")
        .as(Reporte._set *)
    }
  }
  ...
  def convertir(id: scala.Long): scala.Long = {
    db.withConnection { implicit connection =>
      ...
      val transformador = SQL("SELECT * FROM siap.transformador_reporte WHERE repo_id = {repo_id}").on('repo_id -> id).as(Reporte._set.single)
      ...
      SQL("""UPDATE siap.transformador_reporte SET rees_id = 10 WHERE repo_id = {repo_id}""")
        .on(
  ...
  def actualizar(
    ...
    db.withConnection { implicit connection =>
      ...
      if (!coau_codigo.trim.isEmpty) {
        SQL(
          """INSERT INTO siap.transformador_reporte_codigo_autorizacion (repo_id, coau_codigo, empr_id) VALUES ({repo_id}, {coau_codigo}, {empr_id})"""
        ).on(
            'repo_id -> reporte.repo_id,
...
/*             if ((eventoActualizado || eventoInsertado) && (estado != 9)) {
  ...
  def borrar(repo_id: scala.Long, usua_id: scala.Long): Boolean = {
    db.withConnection { implicit connection =>
      ...
      val count: scala.Long =
        SQL("UPDATE siap.transformador_reporte SET rees_id = 9 WHERE repo_id = {repo_id}")
          .on(
            'repo_id -> repo_id
          )
          .executeUpdate()
              ...
              if (e.aap_id != None) {
                ...
                if (estado != 9) {
                  ...
                  elemento.tiel_id match {
                    case Some(1) =>
                      SQL(
                        """UPDATE siap.transformador_elemento SET aap_bombillo = {aap_bombillo} where aap_id = {aap_id} and empr_id = {empr_id}"""
                      ).on(
                          'aap_bombillo -> e.even_codigo_instalado,
                          'aap_id -> e.aap_id,
                          'empr_id -> reporte.empr_id
                    ...
                    case Some(2) =>
                      SQL(
                        """UPDATE siap.transformador_elemento SET aap_balasto = {aap_balasto} where aap_id = {aap_id} and empr_id = {empr_id}"""
                      ).on(
                          'aap_balasto -> e.even_codigo_instalado,
                          'aap_id -> e.aap_id,
                          'empr_id -> reporte.empr_id
                      ...
                      if (!updated) {
                        SQL(
                          """INSERT INTO siap.transformador_elemento_historia (
                                                    aap_id,
                                                    aael_fecha,
                                                    aap_bombillo_retirado,
                                                    aap_bombillo_instalado,
                    ...
                    case Some(3) =>
                      SQL(
                        """UPDATE siap.transformador_elemento SET aap_arrancador = {aap_arrancador} where aap_id = {aap_id} and empr_id = {empr_id}"""
                      ).on(
                          'aap_arrancador -> e.even_codigo_instalado,
                          'aap_id -> e.aap_id,
                          'empr_id -> reporte.empr_id
                    ...
                    case Some(4) =>
                      SQL(
                        """UPDATE siap.transformador_elemento SET aap_condensador = {aap_condensador} where aap_id = {aap_id} and empr_id = {empr_id}"""
                      ).on(
                          'aap_condensador -> e.even_codigo_instalado,
                          'aap_id -> e.aap_id,
                          'empr_id -> reporte.empr_id
                    ...
                    case Some(5) => {
                      SQL(
                        """UPDATE siap.transformador_elemento SET aap_fotocelda = {aap_fotocelda} where aap_id = {aap_id} and empr_id = {empr_id}"""
                      ).on(
                          'aap_fotocelda -> e.even_codigo_instalado,
                          'aap_id -> e.aap_id,
                          'empr_id -> reporte.empr_id
  ...
  def estados(): Future[Iterable[ReporteEstado]] =
    Future[Iterable[ReporteEstado]] {
      db.withConnection { implicit connection =>
        SQL("""SELECT * FROM siap.transformador_reporte_estado WHERE rees_estado < 9""").as(
  ...
  def tipos(): Future[Iterable[ReporteTipo]] = Future[Iterable[ReporteTipo]] {
    db.withConnection { implicit connection =>
      SQL(
        """SELECT * FROM siap.transformador_reporte_tipo WHERE reti_estado < 9 ORDER BY reti_id"""
  ...
  def formato(reti_id: scala.Long, empr_id: scala.Long): Array[Byte] = {
    ...
    db.withConnection { implicit connection =>
      val tipoReporte: ReporteTipo = SQL(
        """SELECT * FROM siap.transformador_reporte_tipo WHERE reti_id = {reti_id}"""
        ...
        case "xls" => {
          ...
          val sheet1 = Sheet(
            ...
            rows = {
              ...
              var j = 2
              val resultSet =
                SQL("""select r.*, a.*, o.*, rt.*, t.*, b.*, ((r.repo_fecharecepcion + interval '48h')::timestamp + (SELECT COUNT(*) FROM siap.festivo WHERE fest_dia BETWEEN r.repo_fecharecepcion and (r.repo_fecharecepcion + interval '48h')) * '1 day'::interval ) as fecha_limite,  c.cuad_descripcion from siap.transformador_reporte r 
                        left join siap.transformador_reporte_adicional a on r.repo_id = a.repo_id
                        left join siap.transformador_reporte_tipo rt on r.reti_id = rt.reti_id
