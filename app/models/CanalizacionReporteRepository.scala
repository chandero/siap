...
class CanalizacionReporteRepository @Inject()(
  ...
  def validarReporteDiligenciado(
    ...
    db.withConnection { implicit connection =>
      ...
      if (reti_id > 1) {
        val reportes = SQL(
          """SELECT r.repo_consecutivo FROM siap.canalizacion_reporte r 
  ...
  def cuenta(empr_id: scala.Long): scala.Long = {
    db.withConnection { implicit connection =>
      val result = SQL(
        "SELECT COUNT(*) AS c FROM siap.canalizacion_reporte WHERE empr_id = {empr_id} and rees_id < 9"
      ).on(
          'empr_id -> empr_id
        )
        .as(SqlParser.scalar[scala.Long].single)
  ...
  def todos(
    ...
    Future[Iterable[Reporte]] {
      db.withConnection { implicit connection =>
        ...
        reps.map { r =>
          val adicional = SQL(
            """SELECT * FROM siap.canalizacion_reporte_adicional WHERE repo_id = {repo_id}"""
          ).on(
              'repo_id -> r.repo_id
            )
            .as(ReporteAdicional.reporteAdicionalSet.singleOpt)
          val eventos = SQL(
            """SELECT * FROM siap.canalizacion_reporte_evento WHERE repo_id = {repo_id} and even_estado < 8"""
          ).on(
              'repo_id -> r.repo_id
            )
            .as(Evento.eventoSet *)
          val meams = SQL(
            """SELECT m.meam_id FROM siap.canalizacion_reporte_medioambiente m WHERE m.repo_id = {repo_id}"""
          ).on(
              'repo_id -> r.repo_id
            )
            .as(scalar[scala.Long].*)
          val direcciones = SQL(
            """SELECT * FROM siap.canalizacion_reporte_direccion WHERE repo_id = {repo_id} and even_estado < 8"""
          ).on(
              'repo_id -> r.repo_id
            )
          ...
          direcciones.map { d =>
            var dat = SQL(
              """SELECT * FROM siap.canalizacion_reporte_direccion_dato WHERE repo_id = {repo_id} and aap_id = {aap_id} and even_id = {even_id}"""
            ).on(
                'repo_id -> d.repo_id,
                'aap_id -> d.aap_id,
                'even_id -> d.even_id
            ...
            var adi = SQL(
              """SELECT * FROM siap.canalizacion_reporte_direccion_dato_adicional WHERE repo_id = {repo_id} and aap_id = {aap_id} and even_id = {even_id}"""
            ).on(
                'repo_id -> d.repo_id,
                'aap_id -> d.aap_id,
                'even_id -> d.even_id
  ...
  def reportes(empr_id: scala.Long): Future[Iterable[Reporte]] =
    Future[Iterable[Reporte]] {
      db.withConnection { implicit connection =>
        var _list: ListBuffer[Reporte] = new ListBuffer[Reporte]()
        val reps = SQL("""SELECT * FROM siap.canalizacion_reporte r 
                                          WHERE r.empr_id = {empr_id} 
    ...
    Future[Iterable[Vencido]] {
      db.withConnection { implicit connection =>
        SQL(
         ...
         o.orig_descripcion, r.repo_nombre, r.repo_telefono, r.repo_direccion, b.barr_descripcion, r.repo_descripcion, r.rees_id from siap.canalizacion_reporte r
        inner join siap.canalizacion_reporte_tipo rt on rt.reti_id = r.reti_id
        left join siap.barrio b on b.barr_id = r.barr_id
  ...
  def buscarPorConsecutivoWeb(
    ...
    db.withConnection { implicit connection =>
      val r = SQL(
        """SELECT * FROM siap.canalizacion_reporte r
  ...
  def buscarPorId(repo_id: scala.Long): Option[Reporte] = {
    db.withConnection { implicit connection =>
      ...
      val eventos = SQL(
        """SELECT * FROM siap.canalizacion_reporte_evento WHERE repo_id = {repo_id} and even_estado < 8 ORDER BY even_id ASC"""
      ).on(
          'repo_id -> repo_id
        )
        .as(Evento.eventoSet *)
      ...
      val adicional = SQL(
        ...
        .as(ReporteAdicional.reporteAdicionalSet.singleOpt)
      val direcciones = SQL(
        """SELECT * FROM siap.canalizacion_reporte_direccion WHERE repo_id = {repo_id} and even_estado < 8 ORDER BY even_id ASC"""
      ).on(
          'repo_id -> repo_id
        )
        .as(ReporteDireccion.reporteDireccionSet *)
    ...
    db.withConnection { implicit connection =>
      ...
      reps.map { r =>
        val adicional = SQL(
          ...
          .as(ReporteAdicional.reporteAdicionalSet.singleOpt)
        val eventos = SQL(
          """SELECT * FROM siap.canalizacion_reporte_evento WHERE repo_id = {repo_id} """
        ).on(
            'repo_id -> r.repo_id
          )
          .as(Evento.eventoSet *)
  ...
  def buscarPorRangoFechaSolucion(
    ...
    db.withConnection { implicit connection =>
      SQL(
        "SELECT * FROM siap.canalizacion_reporte WHERE repo_fechasolucion BETWEEN {fecha_inicial} and {fecha_final} and empr_id = {empr_id}"
      ).on(
  ...
  def buscarPorEstado(
    ...
    db.withConnection { implicit connection =>
      SQL(
        "SELECT * FROM siap.canalizacion_reporte WHERE rees_id = {rees_id} and empr_id = {empr_id}"
      ).on(
          'rees_id -> rees_id,
          'empr_id -> empr_id
        )
  ...
  def buscarPorActividad(
    ...
    db.withConnection { implicit connection =>
      SQL(
        "SELECT * FROM siap.canalizacion_reporte WHERE acti_id = {acti_id} and empr_id = {empr_id}"
      ).on(
          'acti_id -> acti_id,
          'empr_id -> empr_id
        )
  ...
  def buscarPorOrigen(
    ...
    db.withConnection { implicit connection =>
      SQL(
        "SELECT * FROM siap.canalizacion_reporte WHERE orig_id = {orig_id} and empr_id = {empr_id}"
      ).on(
          'orig_id -> orig_id,
          'empr_id -> empr_id
        )
  ...
  def buscarAvanzado(
    ...
    db.withConnection { implicit connection =>
      SQL("SELECT * FROM siap.canalizacion_reporte")
        .as(simple *)
    }
  }
      ...
      db.withConnection { implicit connection =>
        ...
        if (consec > 0) {
          ...
          reporte.adicional.map { adicional =>
            SQL("""INSERT INTO siap.canalizacion_reporte_adicional (repo_id, 
                                                               repo_fechadigitacion, 
                                                               repo_tipo_expansion, 
                                                               repo_luminaria, 
                                                               repo_redes, 
          ...
          SQL(
            "INSERT INTO siap.auditoria(audi_fecha, audi_hora, usua_id, audi_tabla, audi_uid, audi_campo, audi_valorantiguo, audi_valornuevo, audi_evento) VALUES ({audi_fecha}, {audi_hora}, {usua_id}, {audi_tabla}, {audi_uid}, {audi_campo}, {audi_valorantiguo}, {audi_valornuevo}, {audi_evento})"
              ...
              'audi_hora -> hora,
              'usua_id -> reporte.usua_id,
              'audi_tabla -> "canalizacion_reporte",
              'audi_uid -> id,
              'audi_campo -> "repo_id",
              'audi_valorantiguo -> "",
              'audi_valornuevo -> id,
  ...
  def convertir(id: scala.Long): scala.Long = {
    db.withConnection { implicit connection =>
      ...
      val control = SQL("SELECT * FROM siap.canalizacion_reporte WHERE repo_id = {repo_id}").on('repo_id -> id).as(simple.single)
      ...
      reporte match {
        case None =>
                ...
                val queryAdicional = """SELECT
                               ...
                               tran_id,
                               medi_acta
                              FROM siap.canalizacion_reporte_adicional a WHERE a.repo_id = {repo_id}
                              """
      ...
      SQL("""UPDATE siap.canalizacion_reporte SET rees_id = 10 WHERE repo_id = {repo_id}""")
        .on(
  ...
  def actualizar(
    ...
    db.withConnection { implicit connection =>
      ...
      val result: Boolean = SQL(
        "UPDATE siap.canalizacion_reporte SET repo_direccion = {repo_direccion}, repo_nombre = {repo_nombre}, repo_telefono = {repo_telefono}, repo_fechasolucion = {repo_fechasolucion}, repo_horainicio = {repo_horainicio}, repo_horafin = {repo_horafin}, repo_reportetecnico = {repo_reportetecnico}, repo_descripcion = {repo_descripcion}, rees_id = {rees_id}, orig_id = {orig_id}, barr_id = {barr_id}, empr_id = {empr_id}, tiba_id = {tiba_id}, usua_id = {usua_id} WHERE repo_id = {repo_id}"
      ).on(
          'repo_id -> reporte.repo_id,
          'repo_direccion -> reporte.repo_direccion,
          'repo_nombre -> reporte.repo_nombre,
      ...
      reporte.adicional.map { adicional =>
        val hayAdicional: Boolean = SQL(
          """UPDATE siap.canalizacion_reporte_adicional SET repo_fechadigitacion = {repo_fechadigitacion}, 
      ...
      reporte.direcciones.map { direcciones =>
        for (d <- direcciones) {
          if (d.aap_id != None) {
            ...
            var aap: Canalizacion = new Canalizacion(d.aap_id, reporte.empr_id, reporte.usua_id, None, None, Some(1), reporte.repo_fechasolucion, None)
        ...
        for (e <- eventos) {
          if (e.aap_id != None) {
            ...
            eventoActualizado = SQL(
              """UPDATE siap.canalizacion_reporte_evento SET 
            ...
            if (!eventoActualizado) {
              eventoInsertado = SQL(
                """INSERT INTO siap.canalizacion_reporte_evento (even_fecha, 
            ...
            if ((eventoActualizado || eventoInsertado) && (estado != 9)) {
              ...
              elemento.tiel_id match {
                case Some(1) =>
                  SQL(
                    """UPDATE siap.canalizacion_elemento SET aap_bombillo = {aap_bombillo}, reti_id = {reti_id} , repo_consecutivo = {repo_consecutivo} where aap_id = {aap_id} and empr_id = {empr_id}"""
                  ).on(
                  ...
                  val updated: Boolean = SQL(
                    """UPDATE siap.canalizacion_elemento_historia SET aap_bombillo_retirado = {aap_bombillo_retirado}, aap_bombillo_instalado = {aap_bombillo_instalado}
                    ...
                    .executeUpdate() > 0
                  if (!updated) {
                    SQL("""INSERT INTO siap.canalizacion_elemento_historia (
                                                    aap_id,
                                                    aael_fecha,
                ...
                case Some(2) =>
                  SQL(
                    """UPDATE siap.canalizacion_elemento SET aap_balasto = {aap_balasto}, reti_id = {reti_id}, repo_consecutivo = {repo_consecutivo} where aap_id = {aap_id} and empr_id = {empr_id}"""
                  ).on(
                    ...
                    .executeUpdate()
                  val updated: Boolean = SQL(
                    """UPDATE siap.canalizacion_elemento_historia SET aap_balasto_retirado = {aap_balasto_retirado}, aap_balasto_instalado = {aap_balasto_instalado}
                                                 WHERE aap_id = {aap_id} and aael_fecha = {aael_fecha} and reti_id = {reti_id} and repo_consecutivo = {repo_consecutivo} and empr_id = {empr_id}"""
                  ).on(
                ...
                case Some(3) =>
                  SQL(
                    """UPDATE siap.canalizacion_elemento SET aap_arrancador = {aap_arrancador}, reti_id = {reti_id}, repo_consecutivo = {repo_consecutivo} where aap_id = {aap_id} and empr_id = {empr_id}"""
                  ).on(
                    ...
                    .executeUpdate()
                  val updated: Boolean = SQL(
                    """UPDATE siap.canalizacion_elemento_historia SET aap_arrancador_retirado = {aap_arrancador_retirado}, aap_arrancador_instalado = {aap_arrancador_instalado}
                                                 WHERE aap_id = {aap_id} and aael_fecha = {aael_fecha} and reti_id = {reti_id} and repo_consecutivo = {repo_consecutivo} and empr_id = {empr_id}"""
                  ).on(
                ...
                case Some(4) =>
                  SQL(
                    """UPDATE siap.canalizacion_elemento SET aap_condensador = {aap_condensador}, reti_id = {reti_id}, repo_consecutivo = {repo_consecutivo} where aap_id = {aap_id} and empr_id = {empr_id}"""
                  ).on(
                    ...
                    .executeUpdate()
                  val updated: Boolean = SQL(
                    """UPDATE siap.canalizacion_elemento_historia SET aap_condensador_retirado = {aap_condensador_retirado}, aap_condensador_instalado = {aap_condensador_instalado}
                                                 WHERE aap_id = {aap_id} and aael_fecha = {aael_fecha} and reti_id = {reti_id} and repo_consecutivo = {repo_consecutivo} and empr_id = {empr_id}"""
                  ).on(
                ...
                case Some(5) =>
                  SQL(
                    """UPDATE siap.canalizacion_elemento SET aap_fotocelda = {aap_fotocelda}, reti_id = {reti_id}, repo_consecutivo = {repo_consecutivo} where aap_id = {aap_id} and empr_id = {empr_id}"""
                  ).on(
                    ...
                    .executeUpdate()
                  val updated: Boolean = SQL(
                    """UPDATE siap.canalizacion_elemento_historia SET aap_fotocelda_retirado = {aap_fotocelda_retirado}, aap_fotocelda_instalado = {aap_fotocelda_instalado}
                                                 WHERE aap_id = {aap_id} and aael_fecha = {aael_fecha} and reti_id = {reti_id} and repo_consecutivo = {repo_consecutivo} and empr_id = {empr_id}"""
                  ).on(
      ...
      reporte.direcciones.map { direcciones =>
        for (d <- direcciones) {
          if (d.aap_id != None) {
            ...
            // Direccion
            dirActualizado = SQL(
              """UPDATE siap.canalizacion_reporte_direccion SET
            ...
            if (!dirActualizado) {
              dirInsertado = SQL(
                """INSERT INTO siap.canalizacion_reporte_direccion (repo_id, aap_id, even_direccion, barr_id, even_id, even_direccion_anterior, barr_id_anterior, even_estado, tire_id) VALUES ({repo_id}, {aap_id}, {even_direccion}, {barr_id}, {even_id}, {even_direccion_anterior}, {barr_id_anterior}, {even_estado}, {tire_id})"""
              ).on(
                  'repo_id -> reporte.repo_id,
                  'aap_id -> d.aap_id,
                  'even_direccion -> d.even_direccion,
            ...
            // Direccion Dato
            datoActualizado = SQL(
              """UPDATE siap.canalizacion_reporte_direccion_dato SET
                            aatc_id = {aatc_id},
              ...
              .executeUpdate() > 0
            if (!datoActualizado) {
              datoInsertado = SQL("""INSERT INTO siap.canalizacion_reporte_direccion_dato (
                                repo_id,
                                even_id,
                                aap_id,
                                aatc_id,
            ...
            datoadicionalActualizado = SQL(
              """UPDATE siap.canalizacion_reporte_direccion_dato_adicional SET 
            ...
            if (!datoadicionalActualizado) {
              datoadicionalInsertado = SQL(
                """INSERT INTO siap.canalizacion_reporte_direccion_dato_adicional (
                                    repo_id,
                                    even_id,
                                    aap_id,
                                    aacu_id_anterior,
            ...
            */
            ...
            if (reporte.reti_id.get == 1 || reporte.reti_id.get == 2 || reporte.reti_id.get == 3) {
              val res = SQL(
                """UPDATE siap.canalizacion SET aap_direccion = {aap_direccion}, barr_id = {barr_id} WHERE aap_id = {aap_id} and empr_id = {empr_id}"""
              ).on(
            ...
            reporte.reti_id match {
              case Some(8) =>
                ...
                d.tire_id match {
                  case Some(3) =>
                    SQL(
                      """UPDATE siap.canalizacion SET esta_id = 9 WHERE aap_id = {aap_id} and empr_id = {empr_id}"""
                    ).on(
                        'aap_id -> d.aap_id,
                        'empr_id -> reporte.empr_id
                      )
                  ...
                  case _ =>
                    SQL(
                      """UPDATE siap.canalizacion SET esta_id = 2 WHERE aap_id = {aap_id} and empr_id = {empr_id}"""
                    ).on(
                        'aap_id -> d.aap_id,
                        'empr_id -> reporte.empr_id
                      )
      ...
      SQL(
        """DELETE FROM siap.canalizacion_reporte_medioambiente WHERE repo_id = {repo_id}"""
      ).on(
          'repo_id -> reporte.repo_id
        )
        .execute()
      ...
      reporte.meams.map { meams =>
        for (m <- meams) {
          SQL(
            """INSERT INTO siap.canalizacion_reporte_medioambiente (repo_id, meam_id) VALUES ({repo_id}, {meam_id})"""
          ).on(
      ...
      if (!coau_codigo.trim.isEmpty) {
        SQL(
          """INSERT INTO siap.canalizacion_reporte_codigo_autorizacion (repo_id, coau_codigo, empr_id) VALUES ({repo_id}, {coau_codigo}, {empr_id})"""
        ).on(
            'repo_id -> reporte.repo_id,
  ...
  def borrar(repo_id: scala.Long, usua_id: scala.Long): Boolean = {
    db.withConnection { implicit connection =>
      ...
      val count: scala.Long =
        SQL("UPDATE siap.canalizacion_reporte SET rees_id = 9 WHERE repo_id = {repo_id}")
          .on(
            'repo_id -> repo_id
          )
          .executeUpdate()
              ...
              if (e.aap_id != None) {
                ...
                if (estado != 9) {
                  ...
                  elemento.tiel_id match {
                    case Some(1) =>
                      SQL(
                        """UPDATE siap.canalizacion_elemento SET aap_bombillo = {aap_bombillo} where aap_id = {aap_id} and empr_id = {empr_id}"""
                      ).on(
                          'aap_bombillo -> e.even_codigo_instalado,
                          'aap_id -> e.aap_id,
                          'empr_id -> reporte.empr_id
                    ...
                    case Some(2) =>
                      SQL(
                        """UPDATE siap.canalizacion_elemento SET aap_balasto = {aap_balasto} where aap_id = {aap_id} and empr_id = {empr_id}"""
                      ).on(
                          'aap_balasto -> e.even_codigo_instalado,
                          'aap_id -> e.aap_id,
                          'empr_id -> reporte.empr_id
                      ...
                      if (!updated) {
                        SQL(
                          """INSERT INTO siap.canalizacion_elemento_historia (
                                                    aap_id,
                                                    aael_fecha,
                                                    aap_bombillo_retirado,
                                                    aap_bombillo_instalado,
                    ...
                    case Some(3) =>
                      SQL(
                        """UPDATE siap.canalizacion_elemento SET aap_arrancador = {aap_arrancador} where aap_id = {aap_id} and empr_id = {empr_id}"""
                      ).on(
                          'aap_arrancador -> e.even_codigo_instalado,
                          'aap_id -> e.aap_id,
                          'empr_id -> reporte.empr_id
                    ...
                    case Some(4) =>
                      SQL(
                        """UPDATE siap.canalizacion_elemento SET aap_condensador = {aap_condensador} where aap_id = {aap_id} and empr_id = {empr_id}"""
                      ).on(
                          'aap_condensador -> e.even_codigo_instalado,
                          'aap_id -> e.aap_id,
                          'empr_id -> reporte.empr_id
                    ...
                    case Some(5) => {
                      SQL(
                        """UPDATE siap.canalizacion_elemento SET aap_fotocelda = {aap_fotocelda} where aap_id = {aap_id} and empr_id = {empr_id}"""
                      ).on(
                          'aap_fotocelda -> e.even_codigo_instalado,
                          'aap_id -> e.aap_id,
                          'empr_id -> reporte.empr_id
  ...
  def estados(): Future[Iterable[ReporteEstado]] =
    Future[Iterable[ReporteEstado]] {
      db.withConnection { implicit connection =>
        SQL("""SELECT * FROM siap.canalizacion_reporte_estado WHERE rees_estado < 9""").as(
  ...
  def tipos(): Future[Iterable[ReporteTipo]] = Future[Iterable[ReporteTipo]] {
    db.withConnection { implicit connection =>
      SQL(
        """SELECT * FROM siap.canalizacion_reporte_tipo WHERE reti_estado < 9 ORDER BY reti_id"""
  ...
  def formato(reti_id: scala.Long, empr_id: scala.Long): Array[Byte] = {
    ...
    db.withConnection { implicit connection =>
      val tipoReporte: ReporteTipo = SQL(
        """SELECT * FROM siap.canalizacion_reporte_tipo WHERE reti_id = {reti_id}"""
        ...
        case "xls" => {
          ...
          val sheet1 = Sheet(
            ...
            rows = {
              ...
              var j = 2
              val resultSet =
                SQL("""select r.*, a.*, o.*, rt.*, t.*, b.*, ((r.repo_fecharecepcion + interval '48h')::timestamp + (SELECT COUNT(*) FROM siap.festivo WHERE fest_dia BETWEEN r.repo_fecharecepcion and (r.repo_fecharecepcion + interval '48h')) * '1 day'::interval ) as fecha_limite,  c.cuad_descripcion from siap.canalizacion_reporte r 
                        left join siap.canalizacion_reporte_adicional a on r.repo_id = a.repo_id
                        left join siap.canalizacion_reporte_tipo rt on r.reti_id = rt.reti_id
