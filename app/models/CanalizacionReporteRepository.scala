      ...
      db.withConnection { implicit connection =>
        ...
        if (consec > 0) {
          ...
          val id: scala.Long = SQL(
            """INSERT INTO siap.canalizacion_reporte (tireuc_id,
                                                      repo_fecharecepcion, 
                                                      repo_direccion, 
                                                      repo_nombre, 
                                                      repo_telefono, 
  ...
  def actualizar(
    ...
    db.withConnection { implicit connection =>
      ...
      val result: Boolean = SQL(
        "UPDATE siap.canalizacion_reporte SET tireuc_id = {tireuc_id}, repo_direccion = {repo_direccion}, repo_nombre = {repo_nombre}, repo_telefono = {repo_telefono}, repo_fechasolucion = {repo_fechasolucion}, repo_horainicio = {repo_horainicio}, repo_horafin = {repo_horafin}, repo_reportetecnico = {repo_reportetecnico}, repo_descripcion = {repo_descripcion}, rees_id = {rees_id}, orig_id = {orig_id}, barr_id = {barr_id}, empr_id = {empr_id}, tiba_id = {tiba_id}, usua_id = {usua_id} WHERE repo_id = {repo_id}"
      ).on(
          'tireuc_id -> reporte.tireuc_id,
