...
object Novedad {
  ...
  val _set = {
    get[Option[Long]]("nove_id") ~
      ...
      case nove_id ~ nove_descripcion ~ nove_estado ~ nove_tipo ~ empr_id ~ usua_id =>
        Novedad(
          nove_id,
          nove_descripcion,
          nove_estado,
...
class NovedadRepository @Inject()(dbapi: DBApi)(
  ...
  def buscarPorDescripcion(
      nove_descripcion: String
  ): Future[Iterable[Novedad]] = Future[Iterable[Novedad]] {
    db.withConnection { implicit connection =>
      SQL(
        "SELECT * FROM siap.novedad WHERE nove_descripcion LIKE %{nove_descripcion}% ORDER BY nove_descripcion"
      ).on(
  ...
  def todos(
    ...
    db.withConnection { implicit connection =>
      if (nove_tipo != 0) {
        SQL(
          "SELECT * FROM siap.novedad WHERE nove_tipo = {nove_tipo} ORDER BY nove_descripcion"
        ).on(
            'nove_tipo -> nove_tipo
          )
          .as(Novedad._set *)
        ...
        SQL(
          "SELECT * FROM siap.novedad ORDER BY nove_descripcion"
        ).as(Novedad._set *)
      }
    }
  }
  ...
  /**
    ...
    * @return Future[Iterable[Novedad]]
    */
  def novedades(nove_tipo: Int): Future[Iterable[Novedad]] =
    Future[Iterable[Novedad]] {
      db.withConnection { implicit connection =>
        if (nove_tipo != 0) {
          SQL(
            "SELECT * FROM siap.novedad WHERE nove_tipo = {nove_tipo} ORDER BY nove_descripcion"
  ...
  def crear(novedad: Novedad): Future[Long] = Future {
    db.withConnection { implicit connection =>
      ...
      val id: Long = SQL(
        "INSERT INTO siap.novedad (nove_descripcion, nove_estado, nove_tipo, empr_id, usua_id) VALUES ({nove_descripcion}, {nove_estado}, {empr_id}, {usua_id})"
      ).on(
  ...
  def actualizar(novedad: Novedad): Boolean = {
    ...
    db.withConnection { implicit connection =>
      ...
      val result: Boolean = SQL(
        "UPDATE siap.novedad SET nove_descripcion = {nove_descripcion}, nove_estado = {nove_estado}, nove_tipo = {nove_tipo},usua_id = {usua_id} WHERE nove_id = {nove_id}"
      ).on(
  ...
  def borrar(nove_id: Long, usua_id: Long): Boolean = {
    db.withConnection { implicit connection =>
      ...
      val count: Long =
        SQL("UPDATE siap.novedad SET nove_estado = 9 WHERE nove_id = {nove_id}")
          .on(
            'nove_id -> nove_id
          )
          .executeUpdate()
