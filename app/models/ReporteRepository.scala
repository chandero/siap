...
case class ReporteAdicional( repo_id: Option[scala.Long],
...
case class ReporteDireccion(repo_id: Option[scala.Long],
...
case class Reporte(repo_id: Option[scala.Long],
...
case class Vencido(repo_id: Option[scala.Long],
                   reti_id: Option[scala.Long],
                   repo_consecutivo: Option[scala.Long],
                   repo_fecharecepcion: Option[DateTime], 
...
case class ReporteEstado(rees_id: Option[scala.Long],
                         rees_descripcion: String,
                         rees_estado: Int,
                         usua_id: scala.Long)

case class RepoElemento(rere_id: Option[scala.Long],
                        rere_descripcion: Option[String],
                        rere_estado: Option[Int],
                        usua_id: Option[scala.Long]
                        )

case class RepoElemDanho(reda_id: Option[scala.Long],
                         reda_descripcion: Option[String],
                         reda_estado: Option[Int],
                         usua_id: Option[Long]
                        )

case class ElementoHistoria(aap_id: Option[scala.Long],
...
object ReporteDireccionDato {
   ...
   val reporteDireccionDatoSet = {
       ...
       get[Option[String]]("reporte_direccion_dato.aap_poste_propietario_anterior") ~       
       ...
       get[Option[String]]("reporte_direccion_dato.aap_collarin_anterior") map {
            case aatc_id ~ 
                 ...
                 aap_brazo_anterior ~ 
                 aap_collarin ~ 
                 aap_collarin_anterior => ReporteDireccionDato(
                            aatc_id,
                            aatc_id_anterior,
                            aama_id,
                            aama_id_anterior,
...
object ReporteDireccionDatoAdicional {
    ...
    val _set = {
        ...
        get[Option[scala.Long]]("tran_id") map {
            ...
            medi_id ~
            tran_id_anterior ~ 
            tran_id => ReporteDireccionDatoAdicional(
                aacu_id_anterior,
                aacu_id,
                aaus_id_anterior,
                aaus_id,
...
object ReporteDireccion {
   ...
   val reporteDireccionSet = {
       ...
       get[Option[scala.Long]]("reporte_direccion.tire_id") map {
           case repo_id ~ aap_id ~ even_direccion ~ barr_id ~ even_direccion_anterior ~ barr_id_anterior ~ even_id ~ even_estado ~ tire_id => ReporteDireccion(
                                                                        repo_id,
                                                                        aap_id,
                                                                        even_direccion,
                                                                        barr_id,
...
object ReporteTipo {
    ...
    val repoTipoSet = {
        ...
        get[scala.Long]("reporte_tipo.usua_id") map {
            case reti_id ~ reti_descripcion ~ reti_consecutivo ~ reti_estado ~ usua_id => ReporteTipo(
                                                                                                        reti_id,
                                                                                                        reti_descripcion,
                                                                                                        reti_consecutivo,
                                                                                                        reti_estado,
...
object ReporteEstado {
    ...
    val oEstado = {
        ...
        get[scala.Long]("reporte_estado.usua_id") map {
            case rees_id ~ rees_descripcion ~ rees_estado ~ usua_id => new ReporteEstado(rees_id, rees_descripcion, rees_estado, usua_id)
        }
    }
...
object RepoElemento {
    ...
    val repoElementoSet = {
        ...
        get[Option[scala.Long]]("repoelemento.usua_id") map {
            case rere_id ~ rere_descripcion ~ rere_estado ~ usua_id => new RepoElemento(rere_id, rere_descripcion, rere_estado, usua_id)
        }
    }
}

class ReporteRepository @Inject()(dbapi: DBApi, eventoService: EventoRepository, elementoService: ElementoRepository, empresaService: EmpresaRepository, usuarioService: UsuarioRepository, aapService: AapRepository)(implicit ec:DatabaseExecutionContext){
   ...
   */
  def validarReporteDiligenciado(reti_id: scala.Long, repo_consecutivo: Int, empr_id:scala.Long): (Boolean, String) = {
      db.withConnection { implicit connection =>
        val lista = ListBuffer[String]()
        if (reti_id > 1) {
            val reportes = SQL("""SELECT r.repo_consecutivo FROM siap.reporte r 
  ...
  */
  def cuenta(empr_id: scala.Long): scala.Long =  {
    db.withConnection{ implicit connection =>
      val result = SQL("SELECT COUNT(*) AS c FROM siap.reporte WHERE empr_id = {empr_id} and rees_id <> 9").
      on(
        'empr_id -> empr_id
      ).as(SqlParser.scalar[scala.Long].single)
      result
  ...
  /**
    ...
    */
  def todos(page_size: scala.Long, current_page: scala.Long, empr_id: scala.Long, orderby: String, filter: String): Future[Iterable[Reporte]] =
    Future[Iterable[Reporte]] {
      db.withConnection { implicit connection =>
        ...
        var query: String = """SELECT *
          ...
          .as(simple *)
          reps.map { r =>
            val adicional = SQL("""SELECT * FROM siap.reporte_adicional WHERE repo_id = {repo_id}""").
                              on(
                                 'repo_id -> r.repo_id
                              ).as(ReporteAdicional.reporteAdicionalSet.singleOpt)
            val eventos = SQL("""SELECT * FROM siap.reporte_evento WHERE repo_id = {repo_id} and even_estado < 8""").
                              on(
                                 'repo_id -> r.repo_id
                                ).as(Evento.eventoSet *)
            val meams = SQL("""SELECT m.meam_id FROM siap.reporte_medioambiente m WHERE m.repo_id = {repo_id}""").
                              on(
                                'repo_id -> r.repo_id
                                ).as(scalar[scala.Long].*)
            val direcciones = SQL("""SELECT * FROM siap.reporte_direccion WHERE repo_id = {repo_id} and even_estado < 8""").
                              on(
                                'repo_id -> r.repo_id
                              ).as(ReporteDireccion.reporteDireccionSet *)
            var _listDireccion = new ListBuffer[ReporteDireccion]()
            direcciones.map { d =>
                var dat = SQL("""SELECT * FROM siap.reporte_direccion_dato WHERE repo_id = {repo_id} and aap_id = {aap_id} and even_id = {even_id}""").
                              on(
                                'repo_id -> d.repo_id,
                                'aap_id -> d.aap_id,
                                'even_id -> d.even_id
                            ...
                            dat match {
                                case None => dat = Some(new ReporteDireccionDato(None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None))
                                case Some(dat) => None
                            }
                var adi = SQL("""SELECT * FROM siap.reporte_direccion_dato_adicional WHERE repo_id = {repo_id} and aap_id = {aap_id} and even_id = {even_id}""").
                              on(
                                'repo_id -> d.repo_id,
                                'aap_id -> d.aap_id,
                                'even_id -> d.even_id
                            ...
                            adi match {
                                case None => adi = Some(new ReporteDireccionDatoAdicional(None, None, None, None, None, None, None, None))
                                case Some(adi) => None
                            }
  ...
  */
  def reportes(empr_id:scala.Long): Future[Iterable[Reporte]] = Future[Iterable[Reporte]] {
      db.withConnection { implicit connection =>
        var _list:ListBuffer[Reporte] = new ListBuffer[Reporte]()
  ...
  def siap_reporte_vencido(empr_id: scala.Long): Future[Iterable[Vencido]] = Future[Iterable[Vencido]]  {
    db.withConnection { implicit connection =>

      SQL("""select o.repo_id, o.repo_fecharecepcion, o.reti_id, o.repo_consecutivo, o.fecha_limite, TRUNC(EXTRACT(epoch FROM o.horas)/3600)::Int as horas, o.orig_descripcion, o.repo_nombre, o.repo_telefono, o.repo_direccion, o.barr_descripcion, o.repo_descripcion, o.rees_id, c.cuad_descripcion
         from
    ...
    */
    def buscarPorId(repo_id: scala.Long) : Option[Reporte] = {
        db.withConnection { implicit connection => 
            ...
            val eventos = SQL("""SELECT * FROM siap.reporte_evento WHERE repo_id = {repo_id} and even_estado < 8 ORDER BY even_id ASC""").
            on(
                'repo_id -> repo_id
            ).as(Evento.eventoSet *)
            val meams = SQL("""SELECT m.meam_id FROM siap.reporte_medioambiente m WHERE m.repo_id = {repo_id}""").
            ...
            val direcciones = SQL("""SELECT * FROM siap.reporte_direccion WHERE repo_id = {repo_id} and even_estado < 8 ORDER BY even_id ASC""").
                              on(
                                'repo_id -> repo_id
                              ).as(ReporteDireccion.reporteDireccionSet *)
            var _listDireccion = new ListBuffer[ReporteDireccion]()
    ...
    * @param empr_id: scala.Long
    */
    def buscarPorConsecutivo(reti_id: scala.Long, repo_consecutivo: scala.Long, empr_id: scala.Long) : Option[Reporte] = {
        db.withConnection { implicit connection => 
            val r = SQL("""SELECT * FROM siap.reporte r
                            INNER JOIN siap.reporte_tipo t on r.reti_id = t.reti_id
                            LEFT JOIN siap.reporte_adicional ra on r.repo_id = ra.repo_id
            ...
            r match {
              case Some(r) =>  val eventos = SQL("""SELECT * FROM siap.reporte_evento WHERE repo_id = {repo_id} and even_estado < 8 ORDER BY even_id ASC""").
                                            on(
                                                'repo_id -> r.repo_id
                                              ).as(Evento.eventoSet *)
                               val meams = SQL("""SELECT m.meam_id FROM siap.reporte_medioambiente m WHERE m.repo_id = {repo_id}""").
    ...
    * @param empr_id: scala.Long
    */
    def buscarPorConsecutivoConsulta(reti_id: scala.Long, repo_consecutivo: scala.Long, empr_id: scala.Long) : Option[ReporteConsulta] = {
        db.withConnection { implicit connection => 
            val r = SQL("""SELECT * FROM siap.reporte r
                            INNER JOIN siap.reporte_tipo t on r.reti_id = t.reti_id
                            LEFT JOIN siap.reporte_adicional ra on r.repo_id = ra.repo_id
            ...
            r match {
              case Some(r) => val adicional = SQL("""SELECT * FROM siap.reporte_adicional WHERE repo_id = {repo_id}""").
                                            on(
                                               'repo_id -> r.repo_id
                                                ).as(ReporteAdicional.reporteAdicionalSet.singleOpt)       
    ...
    * @param empr_id: scala.Long
    */
    def buscarPorRango(anho: Int, mes: Int, empr_id: scala.Long) : Future[Iterable[Reporte]] = Future[Iterable[Reporte]] {
        db.withConnection { implicit connection => 
        var _list:ListBuffer[Reporte] = new ListBuffer[Reporte]()
        var query: String = """SELECT *
                                          FROM siap.reporte r 
          ...
          reps.map { r =>
            ...
            val eventos = SQL("""SELECT * FROM siap.reporte_evento WHERE repo_id = {repo_id} """).
                              on(
                                 'repo_id -> r.repo_id
                                ).as(Evento.eventoSet *)
            val meams = SQL("""SELECT m.meam_id FROM siap.reporte_medioambiente m WHERE m.repo_id = {repo_id}""").
            ...
            var _listDireccion = new ListBuffer[ReporteDireccion]()
            direcciones.map { d =>
                val dat = SQL("""SELECT * FROM siap.reporte_direccion_dato WHERE repo_id = {repo_id} and aap_id = {aap_id} and even_id = {even_id}""").
                              on(
                                'repo_id -> d.repo_id,
                                'aap_id -> d.aap_id,
                                'even_id -> d.even_id
                            ).as(ReporteDireccionDato.reporteDireccionDatoSet.singleOpt)
                val adi = SQL("""SELECT * FROM siap.reporte_direccion_dato_adicional WHERE repo_id = {repo_id} and aap_id = {aap_id} and even_id = {even_id}""").
                            on(
                              'repo_id -> d.repo_id,
                              'aap_id -> d.aap_id,
                              'even_id -> d.even_id
    ...
    * @param empr_id: scala.Long
    */
    def buscarPorRangoFechaSolucion(fecha_inicial: DateTime, fecha_final: DateTime, empr_id: scala.Long) : Future[Iterable[Reporte]] = Future[Iterable[Reporte]] {
        db.withConnection { implicit connection => 
            SQL("SELECT * FROM siap.reporte WHERE repo_fechasolucion BETWEEN {fecha_inicial} and {fecha_final} and empr_id = {empr_id}").
            on(
    ...
    * @param empr_id: scala.Long
    */
    def buscarPorEstado(rees_id: scala.Long, empr_id: scala.Long) : Future[Iterable[Reporte]] = Future[Iterable[Reporte]] {
        db.withConnection { implicit connection => 
            SQL("SELECT * FROM siap.reporte WHERE rees_id = {rees_id} and empr_id = {empr_id}").
            on(
                'rees_id -> rees_id,
                'empr_id -> empr_id
            ).as(simple *)
    ...
    * @param empr_id: scala.Long
    */
    def buscarPorActividad(acti_id: scala.Long, empr_id: scala.Long) : Future[Iterable[Reporte]] = Future[Iterable[Reporte]] {
        db.withConnection { implicit connection => 
            SQL("SELECT * FROM siap.reporte WHERE acti_id = {acti_id} and empr_id = {empr_id}").
            on(
                'acti_id -> acti_id,
                'empr_id -> empr_id
            ).as(simple *)
    ...
    * @param empr_id: scala.Long
    */
    def buscarPorOrigen(orig_id: scala.Long, empr_id: scala.Long) : Future[Iterable[Reporte]] = Future[Iterable[Reporte]] {
        db.withConnection { implicit connection => 
            SQL("SELECT * FROM siap.reporte WHERE orig_id = {orig_id} and empr_id = {empr_id}").
            on(
                'orig_id -> orig_id,
                'empr_id -> empr_id
            ).as(simple *)
    ...
    */
    def buscarAvanzado(filtro: scala.collection.mutable.Map[String, Object]) : Future[Iterable[Reporte]] = Future[Iterable[Reporte]] {
        db.withConnection { implicit connection => 
    ...
    * Crear Reporte
    */
    def crear(reporte: Reporte): Future[(scala.Long, scala.Long)] = Future[(scala.Long, scala.Long)] {
        db.withConnection { implicit connection => 
            ...
            if (consec > 0) {
              val id: scala.Long = SQL("INSERT INTO siap.reporte (repo_fecharecepcion, repo_direccion, repo_nombre, repo_telefono, repo_fechasolucion, repo_horainicio, repo_horafin, repo_reportetecnico, repo_descripcion, rees_id, orig_id, barr_id, empr_id, tiba_id, usua_id, reti_id, repo_consecutivo) VALUES ({repo_fecharecepcion}, {repo_direccion}, {repo_nombre}, {repo_telefono}, {repo_fechasolucion}, {repo_horainicio}, {repo_horafin}, {repo_reportetecnico}, {repo_descripcion}, {rees_id}, {orig_id}, {barr_id}, {empr_id}, {tiba_id}, {usua_id}, {reti_id}, {repo_consecutivo})").
              on(
                'repo_fecharecepcion -> reporte.repo_fecharecepcion,
                'repo_direccion -> reporte.repo_direccion, 
                'repo_nombre -> reporte.repo_nombre, 
    ...
    */
    def actualizar(reporte: Reporte, coau_tipo: Int, coau_codigo: String) : Boolean = {
        ...
        db.withConnection { implicit connection => 
            ...
            val result: Boolean = SQL("UPDATE siap.reporte SET repo_direccion = {repo_direccion}, repo_nombre = {repo_nombre}, repo_telefono = {repo_telefono}, repo_fechasolucion = {repo_fechasolucion}, repo_horainicio = {repo_horainicio}, repo_horafin = {repo_horafin}, repo_reportetecnico = {repo_reportetecnico}, repo_descripcion = {repo_descripcion}, rees_id = {rees_id}, orig_id = {orig_id}, barr_id = {barr_id}, empr_id = {empr_id}, tiba_id = {tiba_id}, usua_id = {usua_id} WHERE repo_id = {repo_id}").
            on(
                'repo_id -> reporte.repo_id,
                'repo_direccion -> reporte.repo_direccion, 
                'repo_nombre -> reporte.repo_nombre, 
            ...
            reporte.eventos.map { eventos =>
                for(e <- eventos) {
                  if (e.aap_id != None) {
                    ...
                    eventoActualizado = SQL("""UPDATE siap.reporte_evento SET 
                    ...
                    if ( (eventoActualizado || eventoInsertado) && (estado != 9) ) {
                            ...
                            elemento.tiel_id match {
                                case Some(1) => SQL("""UPDATE siap.aap_elemento SET aap_bombillo = {aap_bombillo}, reti_id = {reti_id} , repo_consecutivo = {repo_consecutivo} where aap_id = {aap_id} and empr_id = {empr_id}""").
                                            on(
                                ...
                                case Some(2) => SQL("""UPDATE siap.aap_elemento SET aap_balasto = {aap_balasto}, reti_id = {reti_id}, repo_consecutivo = {repo_consecutivo} where aap_id = {aap_id} and empr_id = {empr_id}""").
                                            on(
                                ...
                                case Some(3) => SQL("""UPDATE siap.aap_elemento SET aap_arrancador = {aap_arrancador}, reti_id = {reti_id}, repo_consecutivo = {repo_consecutivo} where aap_id = {aap_id} and empr_id = {empr_id}""").
                                            on(
                                ...
                                case Some(4) => SQL("""UPDATE siap.aap_elemento SET aap_condensador = {aap_condensador}, reti_id = {reti_id}, repo_consecutivo = {repo_consecutivo} where aap_id = {aap_id} and empr_id = {empr_id}""").
                                            on(
                                ...
                                case Some(5) => SQL("""UPDATE siap.aap_elemento SET aap_fotocelda = {aap_fotocelda}, reti_id = {reti_id}, repo_consecutivo = {repo_consecutivo} where aap_id = {aap_id} and empr_id = {empr_id}""").
                                            on(
            ...
            reporte.direcciones.map { direcciones => 
                for(d <- direcciones) {
                  if (d.aap_id != None) {
                    ...
                    // Direccion
                    dirActualizado = SQL("""UPDATE siap.reporte_direccion SET
                    ...
                    if (!dirActualizado) {
                        dirInsertado = SQL("""INSERT INTO siap.reporte_direccion (repo_id, aap_id, even_direccion, barr_id, even_id, even_direccion_anterior, barr_id_anterior, even_estado, tire_id) VALUES ({repo_id}, {aap_id}, {even_direccion}, {barr_id}, {even_id}, {even_direccion_anterior}, {barr_id_anterior}, {even_estado}, {tire_id})""").
                        on(
                            'repo_id -> reporte.repo_id,
                            'aap_id -> d.aap_id,
                            'even_direccion -> d.even_direccion,
                    ...
                    // Fin Direccion
                    // Direccion Dato
                    datoActualizado = SQL("""UPDATE siap.reporte_direccion_dato SET
                            aatc_id = {aatc_id},
                    ...
                    datoadicionalActualizado = SQL("""UPDATE siap.reporte_direccion_dato_adicional SET 
                    ...
                    if (!datoadicionalActualizado) { 
                        datoadicionalInsertado =  SQL("""INSERT INTO siap.reporte_direccion_dato_adicional (
                                    repo_id,
                                    even_id,
                                    aap_id,
                                    aacu_id_anterior,
                    ...
                    d.dato match {
                        case Some(dato) =>
                             dato.aatc_id match {
                                case Some(aatc_id) => println("Actualizando Tipo Luminaria")
                                                       SQL("UPDATE siap.aap SET aatc_id = {aatc_id} WHERE aap_id = {aap_id} and empr_id = {empr_id}").
                                                       on(
                                                          'aatc_id -> aatc_id,
                                                          'aap_id -> d.aap_id,
                                                          'empr_id -> reporte.empr_id
                             ...
                             dato.aama_id match {
                                case Some(aama_id) =>  println("Actualizando Marca Luminaria")
                                                       SQL("UPDATE siap.aap SET aama_id = {aama_id} WHERE aap_id = {aap_id} and empr_id = {empr_id}").
                                                       on(
                                                          'aama_id -> aama_id,
                                                          'aap_id -> d.aap_id,
                                                          'empr_id -> reporte.empr_id
                             ...
                             dato.aamo_id match {
                                case Some(aamo_id) =>  println("Actualizando Modelo Luminaria")
                                                       SQL("UPDATE siap.aap SET aamo_id = {aamo_id} WHERE aap_id = {aap_id} and empr_id = {empr_id}").
                                                       on(
                                                          'aamo_id -> aamo_id,
                                                          'aap_id -> d.aap_id,
                                                          'empr_id -> reporte.empr_id
                             ...
                             dato.aap_potencia match {
                                case Some(aap_potencia) => println("Actualizando Tipo Luminaria")
                                                       SQL("UPDATE siap.aap_adicional SET aap_potencia = {aap_potencia} WHERE aap_id = {aap_id} and empr_id = {empr_id}").
                                                       on(
                                                          'aap_potencia -> aap_potencia,
                                                          'aap_id -> d.aap_id,
                                                          'empr_id -> reporte.empr_id
                             ...
                             dato.aap_tecnologia match {
                                case Some(aap_tecnologia) => println("Actualizando Tecnologia")
                                                       SQL("UPDATE siap.aap_adicional SET aap_tecnologia = {aap_tecnologia} WHERE aap_id = {aap_id} and empr_id = {empr_id}").
                                                       on(
                                                          'aap_tecnologia -> aap_tecnologia,
                                                          'aap_id -> d.aap_id,
                                                          'empr_id -> reporte.empr_id
                             ...
                             dato.aap_brazo match {
                                case Some(aap_brazo) => println("Actualizando Brazo")
                                                       SQL("UPDATE siap.aap_adicional SET aap_brazo = {aap_brazo} WHERE aap_id = {aap_id} and empr_id = {empr_id}").
                                                       on(
                                                          'aap_brazo -> aap_brazo,
                                                          'aap_id -> d.aap_id,
                                                          'empr_id -> reporte.empr_id
                             ...
                             dato.aap_collarin match {
                                case Some(aap_collarin) => println("Actualizando Collarin")
                                                       SQL("UPDATE siap.aap_adicional SET aap_collarin = {aap_collarin} WHERE aap_id = {aap_id} and empr_id = {empr_id}").
                                                       on(
                             ...
                             dato.tipo_id match {
                                case Some(tipo_id) => println("Actualizando Tipo Poste")
                                                       SQL("UPDATE siap.aap_adicional SET tipo_id = {tipo_id} WHERE aap_id = {aap_id} and empr_id = {empr_id}").
                                                       on(
                                                          'tipo_id -> tipo_id,
                                                          'aap_id -> d.aap_id,
                                                          'empr_id -> reporte.empr_id
                             ...
                             dato.aap_poste_altura match {
                                case Some(aap_poste_altura) => println("Actualizando Poste Altura")
                                                       SQL("UPDATE siap.aap_adicional SET aap_poste_altura = {aap_poste_altura} WHERE aap_id = {aap_id} and empr_id = {empr_id}").
                                                       on(
                                                          'aap_poste_altura -> aap_poste_altura,
                                                          'aap_id -> d.aap_id,
                                                          'empr_id -> reporte.empr_id
                             ...
                             dato.aap_poste_propietario match {
                                case Some(aap_poste_propietario) => println("Actualizando Poste Propietario")
                                                       SQL("UPDATE siap.aap_adicional SET aap_poste_propietario = {aap_poste_propietario} WHERE aap_id = {aap_id} and empr_id = {empr_id}").
                                                       on(
                                                          'aap_poste_propietario -> aap_poste_propietario,
                                                          'aap_id -> d.aap_id,
                                                          'empr_id -> reporte.empr_id
                             ...
                             dato.aaco_id match {
                                case Some(aaco_id) => println("Actualizando Tipo Medida")
                                                       SQL("UPDATE siap.aap SET aaco_id = {aaco_id} WHERE aap_id = {aap_id} and empr_id = {empr_id}").
                                                       on(
                                                          'aaco_id -> aaco_id,
                                                          'aap_id -> d.aap_id,
                                                          'empr_id -> reporte.empr_id
                    ...
                    d.dato_adicional match {
                        case Some(dato_adicional) => 
                            dato_adicional.aacu_id match {
                                case Some(aacu_id) => println("Actualizando Cuenta Alumbrado")
                                                       SQL("UPDATE siap.aap SET aacu_id = {aacu_id} WHERE aap_id = {aap_id} and empr_id = {empr_id}").
                                                       on(
                                                          'aacu_id -> aacu_id,
                                                          'aap_id -> d.aap_id,
                                                          'empr_id -> reporte.empr_id
                            ...
                            dato_adicional.aaus_id match {
                                case Some(aaus_id) => println("Actualizando Uso")
                                                       SQL("UPDATE siap.aap SET aaus_id = {aaus_id} WHERE aap_id = {aap_id} and empr_id = {empr_id}").
                                                       on(
                                                          'aaus_id -> aaus_id,
                                                          'aap_id -> d.aap_id,
                                                          'empr_id -> reporte.empr_id
                            ...
                            dato_adicional.medi_id match {
                                case Some(medi_id) => println("Actualizando Medidor")
                                                       val actualizar = SQL("UPDATE siap.aap_medidor SET medi_id = {medi_id} WHERE aap_id = {aap_id} and empr_id = {empr_id}").
                                                       on(
                                                          'medi_id -> medi_id,
                                                          'aap_id -> d.aap_id,
                                                          'empr_id -> reporte.empr_id
                                                       ...
                                                       if (!actualizar) {
                                                           val insertado = SQL("INSERT INTO siap.aap_medidor (aap_id, empr_id, medi_id) VALUES ({aap_id}, {empr_id}, {medi_id});").
                                                                            on(
                                                                                'medi_id -> medi_id,
                                                                                'aap_id -> d.aap_id,
                                                                                'empr_id -> reporte.empr_id
                            ...
                            dato_adicional.tran_id match {
                                case Some(tran_id) => println("Actualizando Transformador")
                                                       val actualizar = SQL("UPDATE siap.aap_transformador SET tran_id = {tran_id} WHERE aap_id = {aap_id} and empr_id = {empr_id}").
                                                       on(
                                                          'tran_id -> tran_id,
                                                          'aap_id -> d.aap_id,
                                                          'empr_id -> reporte.empr_id
                                                       ...
                                                       if (!actualizar) {
                                                           val insertado = SQL("INSERT INTO siap.aap_transformador (aap_id, empr_id, tran_id) VALUES ({aap_id}, {empr_id}, {tran_id});").
                                                                            on(
                                                                                'tran_id -> tran_id,
                                                                                'aap_id -> d.aap_id,
                                                                                'empr_id -> reporte.empr_id
                    ...
                    if (reporte.reti_id.get == 3) {
                        val res = SQL("""UPDATE siap.aap SET aap_direccion = {aap_direccion}, barr_id = {barr_id} WHERE aap_id = {aap_id} and empr_id = {empr_id}""").
                        on(
                    ...
                    reporte.reti_id match {
                        case Some(8) =>  
                         SQL("""UPDATE siap.aap SET aaco_id = 3 WHERE aap_id = {aap_id} and empr_id = {empr_id}""").
                         on(
                             'aap_id -> d.aap_id,
                             'empr_id -> reporte.empr_id
                         ).executeUpdate()
                         ...
                         d.tire_id match {
                           case Some(3) =>
                             SQL("""UPDATE siap.aap SET esta_id = 9 WHERE aap_id = {aap_id} and empr_id = {empr_id}""").
                             on(
                                 'aap_id -> d.aap_id,
                                 'empr_id -> reporte.empr_id
                             ).executeUpdate()
                           case _ => 
                             SQL("""UPDATE siap.aap SET esta_id = 1 WHERE aap_id = {aap_id} and empr_id = {empr_id}""").
                             on(
                                 'aap_id -> d.aap_id,
                                 'empr_id -> reporte.empr_id
                             ).executeUpdate()                            
            ...
            SQL("""DELETE FROM siap.reporte_medioambiente WHERE repo_id = {repo_id}""").
            on(
                'repo_id -> reporte.repo_id
            ).execute()
            ...
            reporte.meams.map { meams =>
                for (m <- meams) {
                    SQL("""INSERT INTO siap.reporte_medioambiente (repo_id, meam_id) VALUES ({repo_id}, {meam_id})""").
                    on(
            ...
            if (!coau_codigo.trim.isEmpty) {
                SQL("""INSERT INTO siap.reporte_codigo_autorizacion (repo_id, coau_codigo, empr_id) VALUES ({repo_id}, {coau_codigo}, {empr_id})""").
                on(
                'repo_id -> reporte.repo_id,
    ...
    */
    def borrar(repo_id: scala.Long, usua_id: scala.Long) : Boolean = {
        db.withConnection { implicit connection => 
            ...
            val count:scala.Long = SQL("UPDATE siap.reporte SET rees_id = 9 WHERE repo_id = {repo_id}").
            on(
                'repo_id -> repo_id
            ).executeUpdate()
                ...
                for(e <- eventos) {
                  if (e.aap_id != None) {
                    ...
                    if ( estado != 9 ) {
                            // validar elemento y actualizar aap_elemento
                            elemento.tiel_id match {
                                case Some(1) => SQL("""UPDATE siap.aap_elemento SET aap_bombillo = {aap_bombillo} where aap_id = {aap_id} and empr_id = {empr_id}""").
                                            on(
                                              'aap_bombillo -> e.even_codigo_instalado,
                                              'aap_id -> e.aap_id,
                                              'empr_id -> reporte.empr_id
                                ...
                                case Some(2) => SQL("""UPDATE siap.aap_elemento SET aap_balasto = {aap_balasto} where aap_id = {aap_id} and empr_id = {empr_id}""").
                                            on(
                                              'aap_balasto -> e.even_codigo_instalado,
                                              'aap_id -> e.aap_id,
                                              'empr_id -> reporte.empr_id
                                ...
                                case Some(3) => SQL("""UPDATE siap.aap_elemento SET aap_arrancador = {aap_arrancador} where aap_id = {aap_id} and empr_id = {empr_id}""").
                                            on(
                                              'aap_arrancador -> e.even_codigo_instalado,
                                              'aap_id -> e.aap_id,
                                              'empr_id -> reporte.empr_id
                                ...
                                case Some(4) => SQL("""UPDATE siap.aap_elemento SET aap_condensador = {aap_condensador} where aap_id = {aap_id} and empr_id = {empr_id}""").
                                            on(
                                              'aap_condensador -> e.even_codigo_instalado,
                                              'aap_id -> e.aap_id,
                                              'empr_id -> reporte.empr_id
                                ...
                                case Some(5) => SQL("""UPDATE siap.aap_elemento SET aap_fotocelda = {aap_fotocelda} where aap_id = {aap_id} and empr_id = {empr_id}""").
                                            on(
                                              'aap_fotocelda -> e.even_codigo_instalado,
                                              'aap_id -> e.aap_id,
                                              'empr_id -> reporte.empr_id
    ...
    */
    def tipos(): Future[Iterable[ReporteTipo]] = Future[Iterable[ReporteTipo]] {
      db.withConnection { implicit connection =>
        SQL("""SELECT * FROM siap.reporte_tipo WHERE reti_estado <> 9 ORDER BY reti_id""").as(ReporteTipo.repoTipoSet *)
      }
    }
      ...
      db.withConnection { implicit connection => 
        empresaService.buscarPorId(empr_id).map { empresa =>
          try {
            ...
            reporte match {
                case Some(r) => if (reporte.get.reti_id.get == 2 || reporte.get.reti_id.get == 6) {
    ...
    */
    def formato(reti_id: scala.Long, empr_id: scala.Long): Array[Byte] = {
      var os = Array[Byte]()
      db.withConnection { implicit connection => 
      val tipoReporte:ReporteTipo = SQL("""SELECT * FROM siap.reporte_tipo WHERE reti_id = {reti_id}""").on('reti_id -> reti_id
      ).as(ReporteTipo.repoTipoSet.single)
        empresaService.buscarPorId(empr_id).map { empresa =>
          try {
    ...
    * @return OutputStream
    */
    def imprimirRelacion(fecha_inicial: Long, fecha_final: Long, empr_id: scala.Long, usua_id: scala.Long, tipo: String): Array[Byte] = {
        ...
        db.withConnection { implicit connection =>
            ...
            tipo match {
                case "pdf" => os = JasperRunManager.runReportToPdf(compiledFile, reportParams, connection)
                case "xls" => {
                    ...
                    var columnstyle = {
                        ...
                        _listColumn += com.norbitltd.spoiwo.model.Column(index = 1, width = new Width(50, WidthUnit.Character))
                        _listColumn += com.norbitltd.spoiwo.model.Column(index = 2, width = new Width(50, WidthUnit.Character))
                        _listColumn += com.norbitltd.spoiwo.model.Column(index = 3, width = new Width(50, WidthUnit.Character))
                        _listColumn.toList
                    }
                    val sheet1 = Sheet(name="HojaT",
                    rows = {
                        val title1Row =com.norbitltd.spoiwo.model.Row(style= CellStyle(font = Font(bold=true), horizontalAlignment = HA.Center)).withCellValues("CONCESIÓN ALUMBRADO PÚBLICO") 
                        val title2Row = com.norbitltd.spoiwo.model.Row(style= CellStyle(font = Font(bold=true), horizontalAlignment = HA.Center)).withCellValues(empresa.empr_descripcion)
                        val title3Row =com.norbitltd.spoiwo.model.Row(style= CellStyle(font = Font(bold=true), horizontalAlignment = HA.Center)).withCellValues("CONTROL ATENCIÓN A USUARIOS") 
                        val title4Row = com.norbitltd.spoiwo.model.Row(style= CellStyle(font = Font(bold=true), horizontalAlignment = HA.Center)).withCellValues("OPERACION Y MANTENIMIENTO PENDIENTE")
                        val headerRow = com.norbitltd.spoiwo.model.Row(style= CellStyle(font = Font(bold=true))).withCellValues("Número de Reporte", "Fecha y Hora", "Fecha Límite","Persona/Entidad Usuario", "Teléfono", "Dirección", "Barrio", "Medio", "Daño Reportado", "Cuadrilla")
                        var j = 2
                        val resultSet = SQL("""select r.*, a.*, o.*, rt.*, t.*, b.*, ((r.repo_fecharecepcion + interval '48h')::timestamp + (SELECT COUNT(*) FROM siap.festivo WHERE fest_dia BETWEEN r.repo_fecharecepcion and (r.repo_fecharecepcion + interval '48h')) * '1 day'::interval ) as fecha_limite,  c.cuad_descripcion from siap.reporte r 
                    ...
                    """).on(
                        ...
                        val rows = resultSet.map { i => j+=1
                                                        com.norbitltd.spoiwo.model.Row(
                                                            NumericCell(i.repo_consecutivo match { case Some(value) => value case None => 0 }, Some(0), style = Some(CellStyle( dataFormat = CellDataFormat("#0"))), CellStyleInheritance.CellThenRowThenColumnThenSheet
                                                            ),
                                                            DateCell(i.repo_fecharecepcion match { case Some(v) => new java.util.Date(v.getMillis) case None => new java.util.Date(0) }, Some(1), style = Some(CellStyle( dataFormat = CellDataFormat("YYYY-MM-DD HH:mm"))), CellStyleInheritance.CellThenRowThenColumnThenSheet
                                                            ),
                                                            DateCell(i.fecha_limite match { case Some(v) => new java.util.Date(v.getMillis) case None => new java.util.Date(0) }, Some(2), style = Some(CellStyle( dataFormat = CellDataFormat("YYYY-MM-DD HH:mm"))), CellStyleInheritance.CellThenRowThenColumnThenSheet
                                                            ),
                                                            StringCell(i.repo_nombre match { case Some(value) => value case None => "" }, Some(3), style = Some(CellStyle( dataFormat = CellDataFormat("@"))), CellStyleInheritance.CellThenRowThenColumnThenSheet
                                                            ),
                                                            StringCell(i.repo_telefono match { case Some(value) => value case None => "" }, Some(4), style = Some(CellStyle( dataFormat = CellDataFormat("@"))), CellStyleInheritance.CellThenRowThenColumnThenSheet
                                                            ),                                                            
                                                            StringCell(i.repo_direccion match { case Some(value) => value case None => "" }, Some(5), style = Some(CellStyle( dataFormat = CellDataFormat("@"))), CellStyleInheritance.CellThenRowThenColumnThenSheet
                                                            ),                                                            
                                                            StringCell(i.barr_descripcion match { case Some(value) => value case None => "" }, Some(6), style = Some(CellStyle( dataFormat = CellDataFormat("@"))), CellStyleInheritance.CellThenRowThenColumnThenSheet
                                                            ),
                                                            StringCell(i.orig_descripcion match { case Some(value) => value case None => "" }, Some(7), style = Some(CellStyle( dataFormat = CellDataFormat("@"))), CellStyleInheritance.CellThenRowThenColumnThenSheet
                                                            ),
                                                            StringCell(i.acti_descripcion match { case Some(value) => value case None => "" }, Some(8), style = Some(CellStyle( dataFormat = CellDataFormat("@"))), CellStyleInheritance.CellThenRowThenColumnThenSheet
                                                            ),
                                                            StringCell(i.cuad_descripcion match { case Some(value) => value case None => "" }, Some(9), style = Some(CellStyle( dataFormat = CellDataFormat("@"))), CellStyleInheritance.CellThenRowThenColumnThenSheet
                                                            )                                                    
                                                        )
                                                 }
    ...
    def consecutivo(reti_id: scala.Long): scala.Long = {
        db.withConnection { implicit connection => 
            val count:scala.Long = SQL("UPDATE siap.reporte_tipo SET reti_consecutivo = reti_consecutivo + 1 WHERE reti_id = {reti_id}").on('reti_id -> reti_id).executeUpdate()
            if(count > 0) {
                val consec = SQL("SELECT reti_consecutivo FROM siap.reporte_tipo WHERE reti_id = {reti_id}").
                                on(
                                    'reti_id -> reti_id
                                ).as(SqlParser.scalar[scala.Long].single)
                consec
