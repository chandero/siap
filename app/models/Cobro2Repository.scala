...
class Cobro2Repository @Inject()(
  ...
  def siap_orden_trabajo_cobro_consecutivo(empr_id: Long): Int = {
    db.withConnection { implicit connection =>
      val query =
        "SELECT gene_numero FROM siap.general WHERE empr_id = {empr_id} and gene_id = 7"
      SQL(query)
        .on(
          'empr_id -> empr_id
        )
  ...
  def siap_obtener_orden_trabajo_cobro_expansion(
      empr_id: Long,
      reti_id: Long
  ...
  def siap_generar_orden_trabajo_cobro_expansion(
      anho: Int,
      mes: Int,
      tireuc_id: Int,
      reti_id: scala.Long,
    ...
    db.withTransaction { implicit connection =>
      ...
      SQL("UPDATE siap.general SET gene_numero = {numero} WHERE gene_id = 7")
        .on(
      ...
      var orden_consecutivo_inicial =
        SQL("""SELECT gene_numero FROM siap.general WHERE gene_id = 7""").as(
      ...
      SQL("""
              ...
              from
                (select cotr1.cotr_id, max(a1.aaus_id) as aaus_id from siap.cobro_orden_trabajo cot1
                  ...
                  inner join siap.reporte_direccion rd1 on rd1.repo_id = r1.repo_id
                  inner join siap.aap a1 on a1.aap_id = rd1.aap_id
                  where rd1.even_estado < 8 and cot1.cotr_tipo_obra = 2 and cot1.cotr_anho = {anho} and cot1.cotr_periodo = {periodo} and cot1.empr_id = {empr_id}
                group by 1) as o
  ...
  def siap_orden_trabajo_cobro(
    ...
    val sheet1 = siap_orden_trabajo_cobro_expansion_hoja_1_orden(
      empresa.get,
      orden.get
    )

    var sheet3 = siap_orden_trabajo_cobro_expansion_hoja_3_unitarios(
      empresa.get,
      orden.get,
      barrio.get,
    ...
    val sheet2 = siap_orden_trabajo_cobro_expansion_hoja_2_cuadro_general(
      empresa.get,
      orden.get,
      barrio.get,
      _listCuadroGeneralUnitario
    ...
    val sheet4 =
      siap_orden_trabajo_cobro_expansion_hoja_4_reporte_luminaria(
        empresa.get,
        orden.get
      )

    val sheet5 =
      siap_orden_trabajo_cobro_expansion_hoja_5_entrega(
        empresa.get,
        orden.get
      )

    val sheet98 =
      siap_orden_trabajo_cobro_expansion_hoja_98_resumen_material(
        empresa.get,
        orden.get,
        _listCuadroGeneralUnitario
      )

    val sheet99 = siap_orden_trabajo_cobro_expansion_hoja_99_generado(
      empresa.get,
      orden.get,
      usua_id
    )
  ...
  def siap_orden_trabajo_cobro_expansion_hoja_1_orden(
      empresa: Empresa,
  ...
  def siap_orden_trabajo_cobro_expansion_hoja_5_entrega(
      empresa: Empresa,
      orden: orden_trabajo_cobro
  ): Sheet = {
    var _listRow01 = new ListBuffer[com.norbitltd.spoiwo.model.Row]()
  ...
  def siap_orden_trabajo_cobro_expansion_hoja_2_cuadro_general(
      empresa: Empresa,
      orden: orden_trabajo_cobro,
      barrio: Barrio,
  ...
  def siap_orden_trabajo_cobro_expansion_hoja_3_unitarios(
      empresa: Empresa,
      orden: orden_trabajo_cobro,
      barrio: Barrio,
      _listCuadroGeneralUnitario: ListBuffer[
    ...
    db.withConnection { implicit connection =>
      ...
      // Validar si se tienen obras
      
      val _hayObras = SQL("""select count(*) from siap.reporte r1
                              inner join siap.reporte_adicional ra1 on ra1.repo_id = r1.repo_id  
                              inner join siap.obra o1 on o1.muot_id = ra1.muot_id and o1.rees_id < 8
                              where r1.repo_id in (select repo_id from siap.cobro_orden_trabajo_reporte cotr1 where cotr1.cotr_id = {cotr_id})
                          """).on('cotr_id -> orden.cotr_id).as(SqlParser.scalar[Long].single) > 0
      ...
      if (_hayObras) { 
        ...
        _nunitarios += "1.06"
        _unitarios = _nunitarios.sorted.toList
      }
      ...
      _unitarios.map { unitario =>
        if (unitario == "1.01") {
          ...
          elements.map { e =>
            var _e: Option[(Int, String)] = None
            if (e._2.startsWith("LUMINARIA")) {
                _e = Some(e)
            }            
            _idx = siap_orden_trabajo_cobro_expansion_hoja_3_unitarios(
              empresa,
              orden,
              barrio,
              unitario,
        ...
        } else if (unitario == "1.06") {
          _idx = siap_orden_trabajo_cobro_expansion_hoja_3_unitarios_obra(
            empresa,
            orden,
            barrio,
            unitario,
  ...
  def siap_orden_trabajo_cobro_expansion_hoja_3_unitarios(
    ...
    println("siap_orden_trabajo_cobro_expansion_hoja_3_unitarios")
 ...
 */          val _qcant = """SELECT SUM(cotrma_cantidad) FROM siap.cobro_orden_trabajo cot1
      ...
      _idx = siap_orden_trabajo_cobro_expansion_hoja_3_unitarios_herramienta(
              empresa,
              orden,
              barrio,
              unit_codigo,
...
/*             if (_m._1.startsWith("FOTOCELDA")) {
      ...
      _idx += 1
      // MANO OBRA
      _idx = siap_orden_trabajo_cobro_expansion_hoja_3_unitarios_mano_obra(
              empresa,
              orden,
              barrio,
              unit_codigo,
  ...
  def siap_orden_trabajo_cobro_expansion_hoja_3_unitarios_obra(
      empresa: Empresa,
      orden: orden_trabajo_cobro,
      barrio: Barrio,
      unit_codigo: String,
    ...
    println("siap_orden_trabajo_cobro_expansion_hoja_3_unitarios_obra")
    val elem_id = element match {
      case Some(e) => Some(e._1)
      case None => None
    }
...
/*       var _cantidad_xls = _cantidad_item match {
      ...
      */
      ...
      val _materiales = {
          var _obras = SQL(
            """select distinct o1.obra_id from siap.reporte r1
                inner join siap.reporte_adicional ra1 on ra1.repo_id = r1.repo_id  
                inner join siap.obra o1 on o1.muot_id = ra1.muot_id and o1.rees_id < 8
                where r1.empr_id = {empr_id} and r1.repo_id in (select repo_id from siap.cobro_orden_trabajo_reporte cotr1 where cotr1.cotr_id = {cotr_id})
            """
          ).on(
              'cotr_id -> orden.cotr_id,
              'empr_id -> empresa.empr_id.get
...
/*             if (_m._1.startsWith("FOTOCELDA")) {
  ...
  def siap_orden_trabajo_cobro_expansion_hoja_3_unitarios_herramienta(
      empresa: Empresa,
      orden: orden_trabajo_cobro,
      barrio: Barrio,
      unit_codigo: String,
    ...
    println("siap_orden_trabajo_cobro_expansion_hoja_3_unitarios_herramienta")
      var _idx = _idx01
      val _fuenteTitulo = Font(
        height = 22.points,
        fontName = "Liberation Sans",
      ...
      println("Finalizado Proceso Hoja 3 Unitarios Herramienta")
      _idx
  }

  def siap_orden_trabajo_cobro_expansion_hoja_3_unitarios_mano_obra(
      empresa: Empresa,
      orden: orden_trabajo_cobro,
      barrio: Barrio,
      unit_codigo: String,
    ...
    println("siap_orden_trabajo_cobro_expansion_hoja_3_unitarios_mano_obra")
      var _idx = _idx01
      val _fuenteTitulo = Font(
        height = 22.points,
        fontName = "Liberation Sans",
      ...
      var _idx_final = _idx + 1
      _manoObra.map { _m =>
        if (!((_m._1 == 4 || _m._1 == 5 || _m._1 == 6) && (unit_codigo == "1.03" || unit_codigo == "1.04"))) {
          _listRow01 += com.norbitltd.spoiwo.model.Row(
          StringCell(
            _m._2,
            Some(1),
  ...
  def siap_orden_trabajo_cobro_expansion_hoja_3_unitarios_material(
      empresa: Empresa,
      orden: orden_trabajo_cobro,
      barrio: Barrio,
      unit_codigo: String,
  ...
  def siap_orden_trabajo_cobro_expansion_hoja_4_reporte_luminaria(
      empresa: Empresa,
      orden: orden_trabajo_cobro
  ): Sheet = {
    println("siap_orden_trabajo_cobro_expansion_hoja_4_reporte_luminaria")
    db.withConnection { implicit connection =>
      var _listRow01 = new ListBuffer[com.norbitltd.spoiwo.model.Row]()
      var _listMerged01 = new ListBuffer[CellRange]()
  ...
  def siap_orden_trabajo_cobro_expansion_hoja_98_resumen_material(
      empresa: Empresa,
      orden: orden_trabajo_cobro,
      _listCuadroGeneralUnitario: ListBuffer[
        (String, String, String, Double, String, String, String, String)
...
/*                   if (_m._2.startsWith("FOTOCELDA")) {
  ...
  def siap_orden_trabajo_cobro_expansion_hoja_99_generado(
      empresa: Empresa,
      orden: orden_trabajo_cobro,
      usua_id: Long
  ): Sheet = {
    println("Procesando hoja 99 - generado")
    db.withConnection { implicit connection =>
