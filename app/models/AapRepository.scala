...
case class Aap_medidor(
    aap_id: Option[Long],
    amet_id: Option[Long],
    aame_numero: Option[String],
    amem_id: Option[Long],
    empr_id: Option[Long],
    medi_id: Option[Long]
)

case class Aap_transformador(
    aap_id: Option[Long],
    empr_id: Option[Long],
    tran_id: Option[Long]
)
  ...
  val aapmedidorSet = {
    get[Option[Long]]("aap_id") ~
      ...
      case aap_id ~
            ...
            empr_id ~
            medi_id =>
        Aap_medidor(aap_id, amet_id, aame_numero, amem_id, empr_id, medi_id)
    }
  }
}
  ...
  val _set = {
    get[Option[Long]]("aap_id") ~
      ...
      case aap_id ~
            empr_id ~
            tran_id =>
        Aap_transformador(aap_id, empr_id, tran_id)
    }
  }
}
...
case class Activo(
    aap: Option[Aap],
    aame: Option[Aap_medidor],
    aatr: Option[Aap_transformador],
    aap_adicional: Option[AapAdicional],
    aap_elemento: Option[AapElemento],
    autorizacion: Option[String],
    esta_id: Option[Int]
)
...
class AapRepository @Inject()(eventoService: EventoRepository, dbapi: DBApi)(
  ...
  def buscarSiguienteACrear(empr_id: scala.Long): Int = {
    db.withConnection { implicit connection =>
      val ultimo = SQL(
        """SELECT a.aap_id FROM siap.aap a WHERE a.empr_id = {empr_id} and a.aap_id <> 9999999
  ...
  def buscarParaVerificar(
      aap_id: Long,
      empr_id: Long
  ): (Int, Iterable[(Int, Long, Int, String)]) = {
    var lista_result: ListBuffer[(Int, Long, Int, String)] = new ListBuffer()
    db.withConnection { implicit connection =>
      var result = 0
      val _aapParser = int("aap_id") ~ int("esta_id") ~ int("aaco_id") map {
        case a ~ e ~ c => (a, e, c)
      }
  ...
  def buscarPorId(aap_id: Long, empr_id: scala.Long): Option[Aap] = {
    db.withConnection { implicit connection =>
      val a = SQL("""SELECT a.*, b.*, t.* FROM siap.aap a
        ...
        .on('aap_id -> aap_id, 'empr_id -> empr_id)
        .as(simple.singleOpt)
      a match {
        case None => None
        case Some(a) =>
          val c = SQL(
            "SELECT * FROM siap.aap_elemento WHERE aap_id = {aap_id} and empr_id = {empr_id}"
          ).on(
              'aap_id -> a.aap_id,
              'empr_id -> empr_id
            )
          ...
          val h = SQL(
            "SELECT * FROM siap.aap_elemento_historia WHERE aap_id = {aap_id} ORDER BY aael_fecha DESC"
          ).on(
              'aap_id -> a.aap_id
            )
  ...
  def buscarParaEditar(aap_id: Long, empr_id: Long): Option[Activo] = {
    db.withConnection { implicit connection =>
      ...
      a match {
        ...
        case Some(a) =>
          val aame = SQL(
            "SELECT * FROM siap.aap_medidor WHERE aap_id = {aap_id} and empr_id = {empr_id}"
          ).on(
              'aap_id -> a.aap_id,
              'empr_id -> a.empr_id
            )
          ...
          val aatr = SQL(
            "SELECT * FROM siap.aap_transformador WHERE aap_id = {aap_id} and empr_id = {empr_id}"
          ).on(
              'aap_id -> a.aap_id,
              'empr_id -> a.empr_id
            )
            .as(Aap_transformador._set.singleOpt)
          val aap_elemento = SQL(
            "SELECT * FROM siap.aap_elemento WHERE aap_id = {aap_id} and empr_id = {empr_id}"
          ).on(
              'aap_id -> a.aap_id,
              'empr_id -> a.empr_id
          ...
          val aap_adicional = SQL(
            "SELECT * FROM siap.aap_adicional WHERE aap_id = {aap_id} and empr_id = {empr_id}"
          ).on(
              'aap_id -> a.aap_id,
              'empr_id -> a.empr_id
            )
          ...
          val estado = SQL(
            """SELECT esta_id FROM siap.aap WHERE aap_id = {aap_id} and empr_id = {empr_id}"""
          ).on(
              'aap_id -> a.aap_id,
              'empr_id -> a.empr_id
            )
            .as(SqlParser.scalar[Int].singleOpt)
          val activo = new Activo(
            Some(aap_n),
            aame,
            aatr,
            aap_adicional,
  ...
  def buscarPorCodigoWeb(aap_id: Long, empr_id: Long): Option[Aap] = {
    db.withConnection { implicit connection =>
      val a = SQL(
        """SELECT a.*, b.*, t.* FROM siap.aap a
               LEFT JOIN siap.barrio b ON a.barr_id = b.barr_id
               LEFT JOIN siap.tipobarrio t ON b.tiba_id = t.tiba_id WHERE aap_id = {aap_id} and empr_id = {empr_id}"""
      ).on('aap_id -> aap_id, 'empr_id -> empr_id).as(simple.singleOpt)
      a match {
        case None => None
        case Some(a) =>
          val c = SQL(
            "SELECT aael.* FROM siap.aap_elemento aael WHERE aap_id = {aap_id} and empr_id = {empr_id}"
          ).on(
              'aap_id -> a.aap_id,
              'empr_id -> empr_id
            )
  ...
  def buscarPorCodigo(aap_codigo: String, empr_id: Long): Option[Aap] = {
    db.withConnection { implicit connection =>
      val a = SQL(
        """SELECT a.*, b.*, t.* FROM siap.aap a
               LEFT JOIN siap.barrio b ON a.barr_id = b.barr_id
               LEFT JOIN siap.tipobarrio t ON b.tiba_id = t.tiba_id WHERE aap_codigo = {aap_codigo} and empr_id = {empr_id} ORDER BY aap_codigo"""
      ).on('aap_codigo -> aap_codigo).as(simple.singleOpt)
      a match {
        case None => None
        case Some(a) =>
          val c = SQL(
  ...
  def buscarPorApoyo(aap_apoyo: String, empr_id: Long): Option[Aap] = {
    db.withConnection { implicit connection =>
      val a = SQL("""SELECT a.*, b.*, t.* FROM siap.aap a
               LEFT JOIN siap.barrio b ON a.barr_id = b.barr_id
               LEFT JOIN siap.tipobarrio t ON b.tiba_id = t.tiba_id
               WHERE aap_apoyo = {aap_apoyo} and empr_id = {empr_id}""")
        .on('aap_apoyo -> aap_apoyo, 'empr_id -> empr_id)
        .as(simple.singleOpt)
      a match {
        case None => None
        case Some(a) =>
          val c = SQL(
            "SELECT aael.* FROM siap.aap_elemento aael WHERE aap_id = {aap_id}"
          ).on(
              'aap_id -> a.aap_id
            )
            .as(AapElemento.aapelementoSet.singleOpt)
  ...
  /**
    ...
    */
  def buscarPorEstado(esta_id: Long, empr_id: Long): Future[Option[Aap]] =
    Future {
      db.withConnection { implicit connection =>
        SQL(
          "SELECT * FROM siap.aap WHERE esta_id = {esta_id} and empr_id = {empr_id} ORDER BY app_codigo"
        ).on('esta_id -> esta_id, 'empr_id -> empr_id).as(simple.singleOpt)
      }
    }(ec)
  ...
  /**
    ...
    */
  def buscarPorBarrio(barr_id: Long, empr_id: Long): Future[Option[Aap]] =
    Future {
      db.withConnection { implicit connection =>
        SQL(
          "SELECT * FROM siap.aap WHERE barr_id = {barr_id} and empr_id = {empr_id} ORDER BY app_codigo"
        ).on('barr_id -> barr_id, 'empr_id -> empr_id).as(simple.singleOpt)
      }
    }(ec)
  ...
  /**
    ...
    * @return Iterable[Aap]
    */
  def buscarPorMaterial(
      codigo: String,
      tiel_id: scala.Long,
      empr_id: scala.Long
  ): Future[Iterable[Aap]] = Future[Iterable[Aap]] {
    db.withConnection { implicit connection =>
  ...
  /**
    ...
    *  @param current_page: Int
    */
  def aaps(empr_id: Long, filter: String): Future[Iterable[Aap]] =
    Future[Iterable[Aap]] {
      var lista_result = new ListBuffer[Aap]
      db.withConnection { implicit connection =>
        var query: String =
  ...
  /**
    ...
    * @param aap: Aap
    */
  def crear(activo: Activo, empr_id: Long, usua_id: Long): Future[Long] =
    Future {
      db.withConnection { implicit connection =>
        val fecha: LocalDate =
        ...
        activo.aame match {
          ...
          case Some(aame) =>
            SQL(
              "INSERT INTO siap.aap_medidor (aap_id, amem_id, amet_id, aame_numero, empr_id, medi_id) VALUES ({aap_id}, {amem_id}, {amet_id}, {aame_numero}, {empr_id}, {medi_id})"
            ).on(
                'aap_id -> aap.aap_id,
                'amem_id -> aame.amem_id,
                'amet_id -> aame.amet_id,
        ...
        activo.aap_elemento match {
          ...
          case Some(aap_elemento) =>
            SQL(
              "INSERT INTO siap.aap_elemento (aap_id, aael_fecha, aap_bombillo, aap_balasto, aap_arrancador, aap_condensador, aap_fotocelda, reti_id, repo_consecutivo, empr_id) VALUES ({aap_id}, {aael_fecha}, {aap_bombillo}, {aap_balasto}, {aap_arrancador}, {aap_condensador}, {aap_fotocelda}, {reti_id}, {repo_consecutivo}, {empr_id})"
            ).on(
                'aap_id -> aap.aap_id,
        ...
        activo.autorizacion match {
          case Some(autorizacion) =>
            SQL(
              """INSERT INTO siap.aap_codigo_autorizacion (aap_id, coau_codigo, empr_id) VALUES ({aap_id}, {coau_codigo}, {empr_id})"""
            ).on(
                'aap_id -> aap.aap_id,
      ...
      activo.aap match {
        ...
        case Some(aap) =>
          ...
          activo.aame match {
            ...
            case Some(aame) =>
              val actualizo = SQL(
                "UPDATE siap.aap_medidor SET medi_id = {medi_id}, amem_id = {amem_id}, amet_id = {amet_id}, aame_numero = {aame_numero} WHERE aap_id = {aap_id} and empr_id = {empr_id}"
              ).on(
                  'aap_id -> aap.aap_id,
                  'amem_id -> aame.amem_id,
                  'amet_id -> aame.amet_id,
          ...
          activo.aap_elemento match {
            ...
            case Some(aap_elemento) =>
              SQL(
                "UPDATE siap.aap_elemento SET aael_fecha = {aael_fecha}, aap_bombillo = {aap_bombillo}, aap_balasto = {aap_balasto}, aap_arrancador = {aap_arrancador}, aap_condensador = {aap_condensador}, aap_fotocelda = {aap_fotocelda}, reti_id = {reti_id}, repo_consecutivo = {repo_consecutivo} WHERE aap_id = {aap_id} and empr_id = {empr_id}"
              ).on(
                  'aap_id -> aap.aap_id,
                  'empr_id -> empr_id,
                  'aael_fecha -> aap_elemento.aael_fecha,
