...
case class ActaDesmonte(
  acde_id: Option[Long],
  acde_numero: Option[Long],
  acde_fecha: Option[DateTime],
  empr_id: Option[Long],
  usua_id: Option[Long],
  acde_fechagenerado: Option[DateTime]
)

object ActaDesmonte {
  val _set = {
    get[Option[Long]]("acde_id") ~
    get[Option[Long]]("acde_numero") ~
    get[Option[DateTime]]("acde_fecha") ~
    get[Option[Long]]("empr_id") ~
    get[Option[Long]]("usua_id") ~
    get[Option[DateTime]]("acde_fechagenerado") map {
      case acde_id ~ acde_numero ~ acde_fecha ~ empr_id ~ usua_id ~ acde_fechagenerado =>
        ActaDesmonte(acde_id, acde_numero, acde_fecha, empr_id, usua_id, acde_fechagenerado)
    }
  }
}

class ActaDesmonteRepository @Inject()(empresaService: EmpresaRepository, usuarioService: UsuarioRepository, dbapi: DBApi)(
  ...
  def ActaDesmonteXls(
    ...
    db.withTransaction { implicit connection =>
      ...
      println("Buscando Acta Numero de Id: " + acde_id + ", Empr_id:" + empr_id)
      val _actaOpt = SQL(
        "SELECT acde_numero, acde_fecha FROM siap.acta_desmonte WHERE acde_id = {acde_id} AND empr_id = {empr_id}"
      ).on(
          'acde_id -> acde_id,
          'empr_id -> empr_id
        )
      ...
      println("Buscando Material de Acta Numero de Id: " + acde_id)
      val _material = SQL(
        """SELECT 
          e1.elem_id, 
          e1.elem_codigo, 
