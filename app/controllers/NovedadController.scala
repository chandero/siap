...
@Singleton
class NovedadController @Inject()(
    service: NovedadRepository,
    cc: ControllerComponents,
    authenticatedUserAction: AuthenticatedUserAction
)(implicit ec: ExecutionContext)
    extends AbstractController(cc) {

  def buscarPorId(tire_id: Long) = authenticatedUserAction.async {
  ...
  def todos(p: Long, c: Long, t: Int) = authenticatedUserAction.async {
    implicit request: Request[AnyContent] =>
  ...
  def novedades(t: Int) = authenticatedUserAction.async {
    implicit request: Request[AnyContent] =>
  ...
  def guardar() = authenticatedUserAction.async {
    implicit request: Request[AnyContent] =>
      ...
      val nnuevo = new Novedad(
        Some(0),
        novedad.nove_descripcion,
        novedad.nove_estado,
        novedad.nove_tipo,
        empr_id,
        usua_id
