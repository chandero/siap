...
@Singleton
class CanalizacionController @Inject()(mService: CanalizacionRepository, cc: ControllerComponents, authenticatedUserAction: AuthenticatedUserAction)(implicit ec: ExecutionContext) extends AbstractController(cc) {
  ...
  def todosEliminados() = authenticatedUserAction.async { implicit request: Request[AnyContent] =>
    ...
    def canalizaciones() = authenticatedUserAction.async { implicit request: Request[AnyContent] =>
        val empr_id = Utility.extraerEmpresa(request)
        mService.canalizaciones(empr_id.get).map { result =>
    ...
    def guardar() = authenticatedUserAction.async { implicit request: Request[AnyContent] =>
      val json = request.body.asJson.get
      var m = json.as[Canalizacion]
      val usua_id = Utility.extraerUsuario(request)
      val empr_id = Utility.extraerEmpresa(request)
      val hora: LocalDateTime =
        new LocalDateTime(Calendar.getInstance().getTimeInMillis())      
      val mnuevo = new Canalizacion(m.aap_id,empr_id, usua_id, m.aap_direccion, m.barr_id, m.esta_id, Some(hora.toDateTime), None)
    ...
    def actualizar() = authenticatedUserAction.async { implicit request: Request[AnyContent] =>
      ...
      val usua_id = Utility.extraerUsuario(request)
      val empr_id = Utility.extraerEmpresa(request)
      val mnuevo = new Canalizacion(m.aap_id, empr_id, usua_id, m.aap_direccion, m.barr_id, m.esta_id, m.aap_fechacreacion, None)
