...
@Singleton
class RedesController @Inject()(mService: RedesRepository, cc: ControllerComponents, authenticatedUserAction: AuthenticatedUserAction)(implicit ec: ExecutionContext) extends AbstractController(cc) {
  ...
  def todosEliminados() = authenticatedUserAction.async { implicit request: Request[AnyContent] =>
    ...
    def redes() = authenticatedUserAction.async { implicit request: Request[AnyContent] =>
        val empr_id = Utility.extraerEmpresa(request)
        mService.redes(empr_id.get).map { result =>
    ...
    def guardar() = authenticatedUserAction.async { implicit request: Request[AnyContent] =>
      ...
      val mnuevo = new Redes(m.aap_id,empr_id, usua_id, m.aap_direccion, m.barr_id, m.esta_id, Some(hora.toDateTime), None)
    ...
    def actualizar() = authenticatedUserAction.async { implicit request: Request[AnyContent] =>
      ...
      val usua_id = Utility.extraerUsuario(request)
      val empr_id = Utility.extraerEmpresa(request)
      val mnuevo = new Redes(m.aap_id, empr_id, usua_id, m.aap_direccion, m.barr_id, m.esta_id, m.aap_fechacreacion, None)
  ...
  def informe_siap_redes(empr_id: scala.Long) = authenticatedUserAction.async { implicit request: Request[AnyContent] =>
     mService.informe_siap_redes(empr_id).map { lista => 
