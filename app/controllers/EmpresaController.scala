...
class EmpresaController @Inject()(
  ...
  def buscarPorId(empr_id: Long) = authenticatedUserAction.async {
  ...
  def guardarEmpresa() = authenticatedUserAction.async {
    implicit request: Request[AnyContent] =>
      val json = request.body.asJson.get
      var empresa = json.as[Empresa]
      val usua_id = Utility.extraerUsuario(request)
      val empresanuevo = new Empresa(
        Some(0),
        empresa.empr_descripcion,
        empresa.empr_sigla,
        empresa.empr_identificacion,
        empresa.empr_direccion,
        empresa.empr_telefono,
        empresa.empr_concesion,
        empresa.empr_estado,
        usua_id.get,
        empresa.depa_id,
        empresa.muni_id,
  ...
  def actualizarEmpresa() = authenticatedUserAction.async {
    implicit request: Request[AnyContent] =>
      val json = request.body.asJson.get
      var empresa = json.as[Empresa]
      val usua_id = Utility.extraerUsuario(request)
  ...
  def borrarEmpresa(id: Long) = authenticatedUserAction.async {
    implicit request: Request[AnyContent] =>
