...
export function getReportesRango(anho, mes) {
  ...
  return request({
    url: '/rere/rang/' + data.anho + '/' + data.mes,
    method: 'get'
  })
}
...
export function getReportePorConsecutivo(reti_id, repo_consecutivo) {
  ...
  return request({
    url: '/rere/gbtc/' + data.reti_id + '/' + data.repo_consecutivo,
    method: 'get'
  })
}
...
export function validarCodigo(elem_id, codigo) {
  ...
  return request({
    url: '/rere/vali/' + data.elem_id + '/' + data.codigo,
    method: 'get'
  })
}
...
export function validarReporteDiligenciado(reti_id, repo_consecutivo) {
  ...
  return request({
    url: '/rere/vadi/' + data.reti_id + '/' + data.repo_consecutivo,
    method: 'get'
  })
}
...
export function convertirReporte(repo_id, reti_id) {
  ...
  return request({
    url: '/rere/conv/' + data.repo_id + '/' + reti_id,
    method: 'get'
  })
}
...
export function printReporte(repo_id, empr_id) {
  ...
  const url = window.location.protocol + '//' + window.location.host.split('/')[0].split(':')[0] + '/api' + '/rere/prn/repo/' + data.repo_id + '/' + data.empr_id
...
export function printReporteRelacion(fecha_inicial, fecha_final, empr_id, usua_id, tipo) {
  ...
  const url = window.location.protocol + '//' + window.location.host.split('/')[0].split(':')[0] + '/api' + '/rere/prn/rela/' + data.fecha_inicial + '/' + data.fecha_final + '/' + data.empr_id + '/' + usua_id + '/' + tipo
...
export function printReporteBlanco(reti_id, empr_id) {
  ...
  const url = window.location.protocol + '//' + window.location.host.split('/')[0].split(':')[0] + '/api' + '/rere/prn/form/' + data.reti_id + '/' + data.empr_id
  window.open(url, '_blank', 'location=no, menubar=no')
}
