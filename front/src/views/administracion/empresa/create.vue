            ...
            <el-row :gutter="4">
              <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
                <el-form-item prop="depa_id" :label="$t('empresa.departamento')">
                  <el-select v-model="empresa.depa_id" name="departamento" :placeholder="$t('departamento.select')" v-on:change="actualizarMunicipios()" filterable remote :remote-method="remoteMethod" :loading="loading">
              ...
              <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
                <el-form-item prop="muni_id" :label="$t('empresa.municipio')">
                  <el-select v-model="empresa.muni_id" name="municipio" :placeholder="$t('municipio.select')">
...
import { saveEmpresa } from '@/api/empresa'
...
export default {
  data() {
    return {
      labelPosition: 'top',
      empresa: {
        empr_descripcion: null,
        empr_sigla: null,
        empr_identificacion: null,
        empr_direccion: null,
        empr_telefono: null,
        empr_concesion: null,
        empr_estado: 1,
  ...
  methods: {
    aplicar() {
      saveEmpresa(this.empresa).then(response => {
    ...
    actualizarMunicipios() {
      if (this.empresa.depa_id) {
        getMunicipios(this.empresa.depa_id).then(response => {
          ...
          console.log('fin get municipios')
        }).catch(error => {
          console.log(error)
        })
      }
    ...
    success() {
      this.$notify({
        ...
        message: this.$i18n.t('usuario.created') + ' ' + this.usuario.usua_apellido + ' ' + this.usuario.usua_nombre + ' -> ' + this.usuario.usua_email,
        type: 'success',
    ...
    error(e) {
      this.$notify.error({
        title: this.$i18n.t('usuario.error'),
        message: this.$i18n.t('usuario.notcreated') + ' ' + e,
