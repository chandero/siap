...
<script>
import { updateMedidor, getMedidor, getMedidorTablaDato } from '@/api/medidor'
...
export default {
  ...
  methods: {
    ...
    getTiposCuenta() {
      getAapCuentasAp().then(response => {
        ...
        getMedidor(this.$route.params.id).then(response => {
          ...
          this.medidor.datos.forEach(d => {
            d.metd_descripcion = this.obtenerMedidorTablaDatoDescripcion(d.metd_id)
          })
        })
      }).catch(() => {})
    },
    ...
    obtenerMedidorTablaDatoDescripcion(id) {
      if (id !== undefined && id !== null) {
        console.log('buscando medidor dato: ' + id)
        return this.medidorTablaDato.find(o => o.metd_id === id, { metd_descripcion: '' }).metd_descripcion
      }
    }
  },
  mounted() {
