<template>
    <el-container>
      <el-main>
      <el-container>
        <el-header class="reporte_header">{{ $t('operativo.reporttitle') }}
        </el-header>
        <el-main>
      ...
      <el-container>
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <span style="margin-left: 10px">{{ scope.row.repo_fecharecepcion | moment('YYYY/MM/DD') }}</span>
          </template>
        </el-table-column>        
        <el-table-column
...
import { getTodos, deleteReporte, getEstados, getTipos, printReporte } from '@/api/reporte'
  ...
  methods: {
    ...
    getReportes() {
      getTodos(this.page_size, this.current_page, this.order, this.qbquery).then(response => {
        this.tableData = response.data.reportes
    ...
    actualizar() {
      if (this.freti_id !== null & this.fconsec !== null) {
        this.sfilter.children[0].query.value = this.freti_id
        this.sfilter.children[1].query.value = this.fconsec
        this.qbquery = this.sfilter
      } else if (this.freti_id !== null) {
        this.tfilter.children[0].query.value = this.freti_id
        this.qbquery = this.tfilter
      } else {
        this.qbquery = null
      }
      this.getReportes()
