...
import { informe_siap_detallado_repotenciacion_xls } from '@/api/informe'
export default {
  data() {
    return {
      labelPosition: 'top',
  ...
  methods: {
    obtenerDatos() {
      this.loading = true
      informe_siap_detallado_repotenciacion_xls(this.fecha_inicial.getTime(), this.fecha_final.getTime()).then(response => {
        this.loading = false
        this.tableData = response.data
      }).catch(error => {
        this.loading = false
        console.log('Error Repotenciacion: ' + error)
      })
    },
    exportarXls() {
      this.downloadLoading = true
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['Fecha Reporte', 'Fecha Digitación', 'Reporte', 'Código Luminaria Anterior', 'Dirección', 'Uso', 'Cuenta Alumbrado', 'Parque', 'Tipo Anterior', 'Pot.(W) Anterior', 'Tecnología Anterior', 'Tipo Actual', 'Pot.(W) Actual', 'Tecnología Actual', 'Tipo Medida']
        ...
        excel.export_json_to_excel(tHeader, data, 'detallado_repotenciacion_' + this.$moment(this.fecha_inicial).format('YYYYMMDD') + '_' + this.$moment(this.fecha_final).format('YYYYMMDD'))
