...
import { informe_siap_detallado_reposicion_xls } from '@/api/informe'
export default {
  data() {
    return {
      labelPosition: 'top',
  ...
  methods: {
    obtenerDatos() {
      informe_siap_detallado_reposicion_xls(this.fecha_inicial.getTime(), this.fecha_final.getTime()).then(response => {
    ...
    exportarXls() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        ...
        excel.export_json_to_excel(tHeader, data, 'detallado_reposicion_' + this.$moment(this.fecha_inicial).format('YYYYMMDD') + '_' + this.$moment(this.fecha_final).format('YYYYMMDD'))
