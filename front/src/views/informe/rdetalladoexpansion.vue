...
export default {
  ...
  methods: {
    obtenerDatos() {
      this.loading = true
      informe_siap_detallado_expansion_xls(this.fecha_inicial.getTime(), this.fecha_final.getTime()).then(response => {
        this.loading = false
    ...
    exportarXls() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['Fecha Instalación', 'Fecha Digitación', 'Reporte Técnico', 'Código Luminaria', 'Dirección', 'Barrio', 'Uso', 'Cuenta Alumbrado', 'Descripción', 'Tipo Poste', 'Tipo', 'Pot.(W)', 'Tecnología', 'Tipo Medida', 'Urbanizadora']
        const filterVal = ['repo_fechasolucion', 'repo_fechasolucion', 'aap_rte', 'aap_id', 'aap_direccion', 'barr_descripcion', 'aaus_descripcion', 'aacu_descripcion', 'repo_descripcion', 'tipo_descripcion', 'aamo_descripcion', 'aap_potencia', 'aap_tecnologia', 'aaco_descripcion', 'urba_descripcion']
        const list = this.tableData
        const data = this.formatJson(filterVal, list)
        excel.export_json_to_excel(tHeader, data, 'detallado_expansion_' + this.$moment(this.fecha_inicial).format('YYYYMMDD') + '_' + this.$moment(this.fecha_final).format('YYYYMMDD'))
