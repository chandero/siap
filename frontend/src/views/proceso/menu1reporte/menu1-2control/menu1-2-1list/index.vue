<template>
    <el-container>
      <el-main>
      <el-container>
        <el-header class="reporte_header">{{ $t('operativo.controltitle') }}
        </el-header>
        <el-main>
          ...
          <el-tabs v-model="activeTab" :tab-position="tabPosition" type="border-card" class="tabs" @tab-click="changeTab" strecth>
            ...
            >
              <el-table
                ...
                stripe
                :default-sort = "{prop: 'repo_consecutivo', order: 'descending'}"
                style="width: 100%"
                max-height="600"
                border>
          <el-table-column
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button
                    ...
                    type="success"
                    :title="$t('print')"
              @click="handlePrint(scope.$index, scope.row)"><i class="el-icon-printer"></i>
            </el-button>
                  </template>
                </el-table-column>
              </el-table>
...
import { getReportesRango, deleteReporte, getEstados, getTipos, printReporte } from '@/api/controlreporte'
...
export default {
  ...
  methods: {
    ...
    handleEdit2 (index, row) {
      this.$router.push({ path: '/proceso/menu1reporte/menu1-2control/menu1-2-3edit/' + row.repo_id })
    ...
    nuevo () {
      this.$router.push({ path: '/proceso/menu1reporte/menu1-2control/menu1-2-2create/2' })
    },
    estado (id) {
    ...
    getData (anho, mes, index, name) {
      this.loading = true
      getReportesRango(anho, mes, 2).then(response => {
