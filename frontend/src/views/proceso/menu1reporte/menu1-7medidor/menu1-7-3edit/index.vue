<template>
  <el-container>
    <el-header>
      <el-row>
        <el-col :span="20">
          <span>{{ $t('route.medidorreporteedit') }} - Estado Actual: {{ estado() }}</span>
        </el-col>
        <el-col :span="4">
          <el-button
            align="right"
    ...
    <el-main>
      <el-form ref="reporteForm" :model="reporte" :rules="rules" :label-position="labelPosition">
        <el-collapse v-model="activePages" @change="handleActivePagesChange">
          ...
          <el-collapse-item name="3" title="DATOS MEDIDOR">
            <div>
              <el-row>
                <el-col :span="24">
                  <el-tag
                    ...
                    :title="'Información Medidor ' + tag.aap_id"
              ...
              >
                <el-row :gutter="4">
                  ...
                  <el-col :xs="24" :sm="10" :md="10" :lg="10" :xl="10">
                    <el-form-item prop="aap_id" label="Código Medidor">
                  ...
                  <el-col v-if="reporte.reti_id !== 0" :xs="24" :sm="7" :md="7" :lg="7" :xl="7">
                    <el-form-item prop="dato_adicional.aap_apoyo" label="Numero Medidor">
                      <el-input
              ...
              <el-col :span="8">
                ...
                <el-row>
                  <el-col v-if="reporte.reti_id !== 0 && false" :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
                  ...
                  <el-col v-if="reporte.reti_id !== 0 & false" :xs="24" :sm="24" :md="8" :lg="8" :xl="8">
                    <el-form-item prop="dato.aatc_id" label="Tipo Medidor">
                      <el-select
                        :disabled="reporte.direcciones[didx].even_estado > 7"
                        style="width:100%;"
                        filterable
            ...
            <el-card :disabled="reporte.direcciones[didx].even_estado > 7">
              <el-row>
                <el-col :span="24">
                  <el-tag
                    ...
                    size="medium"
                    @click="handleTag(tag.idx)"
                    :title="'Material Medidor ' + tag.aap_id"
                    style="cursor: pointer;"
                  >M: {{tag.aap_id}}</el-tag>
                  <el-input
                    class="input-new-address"
              ...
              <el-row>
                <el-col :span="24">
                  <span>MATERIAL MEDIDOR {{ reporte.direcciones[didx].aap_id }}</span>
                </el-col>
              </el-row>
              <el-row :gutter="4" class="hidden-sm-and-down">
                <el-col :md="1" :lg="1" :xl="1">
                ...
                <el-col :md="3" :lg="3" :xl="3">
                  <span style="font-weight: bold;">Código del Medidor</span>
                </el-col>
    ...
    <el-dialog
      title="Convertir Reporte de Medidor a Reporte de Luminaria"
...
import { getAapEdit, getAapEditByNumber, getAapValidar, validar, buscarSiguiente, getMedidors } from '@/api/medidor'
...
export default {
  ...
  methods: {
    ...
    validateAap (direccion, id) {
      if (direccion.aap_id) {
        console.log('Validando Medidor por aap_id')
        ...
        for (var i = 0; i < this.reporte.direcciones.length; i++) {
          ...
          if (
            ...
            const msg = 'Código de medidor ya está incluido en el reporte'
            this.alerta(msg)
        ...
        if (direccion.aap_id) {
          ...
          getAapEdit(direccion.aap_id)
            .then((response) => {
              console.log('Ingreso a getAapEdit Medidor, respuesta:', response.data)
              const activo = {
                aap: {
                  aap_id: response.data.aap_id,
                  aap_direccion: response.data.aap_direccion,
                  barr_id: response.data.barr_id,
                  aama_id: response.data.aama_id,
                  aamo_id: response.data.aamo_id,
                  aacu_id: response.data.aacu_id
                },
                aap_adicional: {
                  aap_potencia: null,
                  aap_tecnologia: null,
        ...
        if (direccion.dato_adicional.aap_apoyo) {
          console.log('Validando Medidor por aap_apoyo')
          // Limpiar Datos Direccion
          direccion.aap_id = null
          direccion.even_direccion_anterior = null
          direccion.barr_id_anterior = null
          ...
          direccion.dato.aap_poste_propietario_anterior = null
          // Fin Limpiar Datos Direccion
          getAapEditByNumber(direccion.dato_adicional.aap_apoyo)
            .then((response) => {
              console.log('Ingreso a getAapEditByNumber Medidor, respuesta:', response.data)
              const activo = {
                aap: {
                  aap_id: response.data.aap_id,
                  aap_apoyo: response.data.aap_apoyo,
                  aap_direccion: response.data.aap_direccion,
                  barr_id: response.data.barr_id,
                  aama_id: response.data.aama_id,
                  aamo_id: response.data.aamo_id,
              ...
              this.aap.aap_id = activo.aap.aap_id
              for (var i = 0; i < this.reporte.direcciones.length; i++) {
                var d = this.reporte.direcciones[i]
                if (
                  d.aap_id === activo.aap.aap_id &&
                  d.even_id !== direccion.even_id
                ) {
                  const msg = 'Código de medidor ya está incluido en el reporte'
                  this.alerta(msg)
              ...
              if (
                ...
                if (this.reporte.reti_id !== 2) {
                  if (direccion.even_estado === 1) {
                    direccion.aap_id = activo.aap.aap_id
                    direccion.even_direccion_anterior =
                      activo.aap.medi_direccion
                    direccion.barr_id_anterior = activo.aap.barr_id
                    direccion.dato.aatc_id_anterior = activo.aap.aatc_id
    ...
    aplicar () {
      ...
      this.reporte.direcciones.forEach((d) => {
        if (
          ...
          */
          // Validar estado de la medidor y tipo de reporte
          ...
          if (this.reporte.reti_id === 3 || this.reporte.reti_id === 7) {
            if (dt.aaco_id_anterior !== 3) {
              ...
              this.$notify.error({
                title: 'Medidor No Retirado',
                message: 'Verifique el medidor: ' + d.aap_id,
                offset: 0
              })
            }
          }
          ...
          if (
            ...
            if (d.esnueva === false) {
              ...
              this.$notify.error({
                title: 'Medidor Ya Existe',
                message: 'Verifique el código del medidor: ' + d.aap_id,
                offset: 0
              })
            }
          }
          ...
          // validar medidor ya retirada
          if (this.reporte.reti_id === 8) {
            if (dt.aaco_id_anterior === 3) {
              validacion = false
              this.$notify.error({
                title: 'Medidor Ya Está Retirado',
                message: 'Verifique el medidor: ' + d.aap_id,
                offset: 0
              })
            }
        ...
        */
        ...
        for (var i = 0; i < this.reporte.direcciones.length; i++) {
          if (this.reporte.direcciones[i].even_valido.aap_id === false && this.reporte.direcciones[i].even_estado < 8) {
            ...
            this.$alert('Código de medidor no válido: ' + this.reporte.direcciones[i].aap_id, 'Error', {
