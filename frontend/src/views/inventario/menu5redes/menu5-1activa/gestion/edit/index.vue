<template>
    <el-container class="box-card">
      <el-header>
          <span>{{ $t('route.redesgestionedit') }}</span>
      </el-header>
    <el-main>
     ...
     <el-footer>
      <el-button ref="submit" :disabled="!validate()" size="medium" type="primary" icon="el-icon-check" @click="aplicar">Guardar Redes</el-button>
     </el-footer>
    </el-container>
</template>
<script>
import { getBarriosEmpresa } from '@/api/barrio'
import { updateAap, getAapEdit } from '@/api/redes'
...
export default {
  ...
  methods: {
    ...
    limpiarAndBack () {
      this.limpiar()
      this.$router.push({ path: '/redes/gestion' })
    },
    limpiar () {
      this.redes = {
        aap_id: null,
