<template>
  <el-container class="gestion-container">
    <el-main>
      <el-container>
        <el-header class="gestion_header">{{ $t('redes.gestion.tabletitle') }}
        </el-header>
        <el-main>
      ...
      <el-container>
        ...
        <el-table-column
          :label="$t('redes.gestion.neighborhood')"
          min-width="250"
          sortable="custom"
          prop="barr_id"
          resizable
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button
              ...
              title="Modificar Red"><i class="el-icon-edit"></i></el-button>
            <el-button
              size="mini"
              circle
              type="danger"
...
import { getTodos, deleteAap } from '@/api/redes'
...
export default {
  ...
  methods: {
    ...
    handleEdit (index, row) {
      this.$router.push({ path: '/inventario/menu5redes/menu5-1activa/gestion/editar/' + row.aap_id })
    ...
    nuevo () {
      this.$router.push({ path: '/inventario/menu5redes/menu5-1activa/gestion/crear' })
    },
    actualizar () {
      this.getAaps()
    },
    ...
    exportarXls () {
      informe_siap_inventario_filtro_xls(new Date().getTime(), this.order, this.qbquery, 1).then(response => {
        ...
        const filename = 'Informe_Redes_Activas.xlsx'
