<template>
  <el-container class="gestion-container">
    <el-main>
      <el-container>
        <el-header class="gestion_header">{{ $t('control.gestion.tabletitle') }}
        </el-header>
        <el-main>
...
import { getTodos, deleteAap } from '@/api/control'
...
import { informe_siap_control_filtro_xls } from '@/api/informe'
...
export default {
  ...
  methods: {
    ...
    exportarXls () {
      informe_siap_control_filtro_xls(new Date().getTime(), btoa(this.order), btoa(JSON.stringify(this.qbquery)), this.empresa.empr_id)
    }
  },
  mounted () {
