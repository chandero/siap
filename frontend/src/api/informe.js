...
export function siap_cuadrilla_consolidado_material_xlsx(fecha_inicial, fecha_final, cuad_id) {
  const data = {
    fecha_inicial,
    fecha_final,
    cuad_id
  ...
  return request({
    url: '/info/sccmxlx/' + data.fecha_inicial + '/' + data.fecha_final + '/' + data.cuad_id,
    method: 'get',
    responseType: 'blob'
  })
}
...
export function informe_siap_general_estadistica_xls (fecha_inicial, fecha_final, formato, empr_id) {
  const data = {
    fecha_inicial,
    fecha_final,
    formato,
